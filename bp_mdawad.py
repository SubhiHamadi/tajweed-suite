...
<title>أحكام مد العوض في القرآن الكريم</title>
<style>
:root {
  --navy:  #00BCD4;
  --navy2: #0A2040;
...
.header-sub { font-size:clamp(10px,2.8vw,13px); color:#FFD54F; margin-bottom:10px; }
...
.author-info { color:#FFD54F; font-size:0.88em; }
...
.help-tab-awad-def.active { background:var(--sila); color:#020B18; border-color:var(--sila); }
.help-tab-awad-ex.active { background:var(--sila); color:#020B18; border-color:var(--sila); }
...
<header>
  <div class="header-title">أحكام مد العوض في القرآن الكريم</div>
  <div class="header-sub">مد العوض — الوقف على التنوين المنصوب يُقرأ ألفاً بحركتين</div>
  <div class="authors">
...
<div class="controls">
  ...
  <div class="ctrl-row" style="justify-content:center;">
    <button class="btn-toggle" onclick="showHelp()" style="background:#EDE7F6;color:#512DA8;border:1.5px solid #9575CD;padding:8px 24px;">📚 تعريفات مد العوض</button>
...
<div class="legend">
  <div class="legend-item" id="leg-مد العوض" style="background:rgba(255,213,79,0.1);color:#FFD54F;border:2px solid #FFD54F" onclick="filterByType('مد العوض',this)">● مد العوض</div>
</div>

<!-- نافذة التعريفات -->
<div class="help-overlay" id="helpOverlay" onclick="hideHelp(event)">
  <div class="help-modal">
    <div class="help-title">📚 تعريفات مد العوض</div>
    <div style="font-size:13px;color:#0D47A1;font-weight:700;text-align:center;margin-bottom:12px;padding:8px;background:rgba(255,255,255,0.92);border-radius:8px;">
      عند الوقف على الكلمة المنوّنة بالفتح (تنوين النصب ً) تُحذف علامة التنوين وتُقرأ الكلمة بألف مدية — مقدارها 2 حركة
    </div>
    <div class="help-tabs">
            <button class="help-tab" style="border-color:#FFD54F;color:#FFD54F" onclick="showTab('awad-def')">تعريف</button>
      <button class="help-tab" style="border-color:#FFD54F;color:#FFD54F" onclick="showTab('awad-ex')">أمثلة</button>
    </div>

            <div class="hcontent show" id="tab-awad-def">
      <div class="help-def"><b style="color:#FFD54F">مد العوض</b><br>
        هاء الضمير المتحركة (للمذكر الغائب المفرد) بين حرفين متحرّكين،
        بشرط أن يكون الحرف الذي يلي الهاء غير همزة.</div>
      <div class="help-letters" style="color:#FFD54F">مقدار المد: 2 حركة</div>
    ...
    <div class="hcontent" id="tab-awad-ex">
      <div class="help-def"><b style="color:#FFD54F">أمثلة من القرآن الكريم</b></div>
      <div class="help-example"><div class="help-ex-label">صلة بالواو (هاء مضمومة):</div>
        ﴿هُدًى﴾ تُقرأ ﴿هُدَا﴾ · ﴿نَارًا﴾ تُقرأ ﴿نَارَا﴾ · ﴿رِزۡقًا﴾ تُقرأ ﴿رِزۡقَا﴾</div>
    </div>
      <div class="help-letters" style="color:#FFD54F">مقدار المد: 2 حركة</div>
...
<div class="table-wrap">
  <div class="table-header">قائمة حالات مد العوض المستخرجة</div>
  <table>
    <thead>
      <tr>
        <th>ت</th>
...
<div id="allCasesOverlay" onclick="closeAllCases(event)">
  <div id="allCasesModal">
    <div id="allCasesTitle">📋 كل حالات مد العوض في هذه الصفحة</div>
...
function showAllCases() {
  if (!currentData || !currentData.cases || !currentData.cases.length) {
    alert('لا يوجد مد العوض موثّق في هذه الآية.');
    return;
  }
...
async function changeAyah(delta) {
  ...
  try {
    const r = await fetch(`/api/awad/nextayah?suraid=${currentData.suraid}&verseid=${currentData.verseid}&delta=${delta}&type=${encodeURIComponent(type)}`);
    const d = await r.json();
    showAyahNavLoading(false);
    if (d.found) {
      const p = {suraid: d.suraid, verseid: d.verseid};
      if (type !== 'الكل') p.type = type;
      const r2 = await fetch('/api/awad/ayah?' + new URLSearchParams(p));
...
async function loadSuras(type) {
  ...
  const r = await fetch(`/api/awad/suras?type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadVerses() {
  ...
  if (!suraid) return;
  const type   = activeFilter || 'الكل';
  const r = await fetch(`/api/awad/verses?suraid=${suraid}&type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadRandom() {
  ...
  const p = {random:1};
  if (type !== 'الكل') p.type = type;
  const r = await fetch('/api/awad/ayah?' + new URLSearchParams(p));
  const d = await r.json();
  if (d.error) return;
  currentData = d;
  renderAyah(d);
...
function renderAyah(d) {
  ...
  // العدادات
  const counts = {'مد العوض':0};
  d.cases.forEach(c => { if (counts[c.itype] !== undefined) counts[c.itype]++; });
  const styleMap = {
    'مد العوض': 'background:#FFF8E1;color:#FFD54F;border:1px solid #FFD54F',
  };
  ...
  // تلوين الكلمات — مد العوض: kseq يشير لموضع الكلمة المنوّنة (كلمة واحدة)
...
function playClipReciter(reciter) {
  ...
  clipPlayer.src = '/api/awad/clip?' + p.toString();
  clipPlayer.play().then(() => { status.textContent = ''; })
    .catch(async () => {
      try {
        const r = await fetch('/api/awad/clip?' + p.toString());
        const d = await r.json();
...
const WORD_QUESTIONS = {
  1: 'ما حكم هذه الكلمة من أحكام مد العوض؟',
...
function openWordExplain(word, sura, verse, color, idx) {
  ...
  clipCtx = (typeof idx === 'number' && !Number.isNaN(idx)) ? {verseid: verse, lo: idx, hi: idx} : null;
  document.getElementById('clipStatus').textContent = '';
  let ruling = '';
...
async function askWordQuestion(num) {
  ...
  try {
    const r = await fetch('/api/awad/explain', {
...
def api_nextayah():
    ...
    try:
        ...
        if itype and itype != 'الكل':
            cur.execute('''SELECT DISTINCT suraid, verseid FROM md_awad
                           WHERE rem1=?
...
@bp.route('/api/awad/suras')
def api_suras():
    itype = request.args.get('type','الكل')
    conn = get_db(); cur = conn.cursor()
    try:
        if itype and itype != 'الكل':
            cur.execute('''SELECT DISTINCT suraid, suraname FROM md_awad
                           WHERE rem1=? ORDER BY CAST(suraid AS INTEGER)''', (itype,))
        else:
            cur.execute('SELECT DISTINCT suraid, suraname FROM md_awad ORDER BY CAST(suraid AS INTEGER)')
        rows = [{'suraid':r[0],'suraname':r[1]} for r in cur.fetchall()]
    except: rows = []
    conn.close(); return jsonify(rows)


@bp.route('/api/awad/verses')
def api_verses():
    ...
    try:
        if itype and itype != 'الكل':
            cur.execute('''SELECT DISTINCT verseid FROM md_awad WHERE suraid=? AND rem1=?
                           ORDER BY CAST(verseid AS INTEGER)''', (suraid, itype))
        else:
            cur.execute('''SELECT DISTINCT verseid FROM md_awad WHERE suraid=? ORDER BY CAST(verseid AS INTEGER)''', (suraid,))
        rows = cur.fetchall()
    except: rows = []
    conn.close(); return jsonify([r[0] for r in rows])


@bp.route('/api/awad/ayah')
def api_ayah():
    suraid  = request.args.get('suraid')
    ...
    if rand == '1':
        try:
            if itype and itype != 'الكل':
                cur.execute('SELECT DISTINCT suraid,verseid FROM md_awad WHERE rem1=? ORDER BY RANDOM() LIMIT 1', (itype,))
            else:
                cur.execute('SELECT DISTINCT suraid,verseid FROM md_awad ORDER BY RANDOM() LIMIT 1')
            r = cur.fetchone()
    ...
    # جلب نص الآية من md_awad (عمود Ayahtext)
    try:
        cur.execute('''SELECT suraname, Ayahtext, pagenum
                       FROM md_awad WHERE suraid=? AND verseid=? LIMIT 1''',
                    (suraid, verseid))
    ...
    cases = [{
        'word'    : c[0],
        'itype'   : 'مد العوض',   # تطبيع rem1 للتوحيد
        'length'  : c[2],
        'position': c[3],
        'color'   : get_color(c[1])
    } for c in cases_raw]
...
@bp.route('/api/awad/explain', methods=['POST'])
def api_explain():
    ...
    try:
        system = ("You are an expert in Quran Tajweed specializing in Madd Al-Muttasil (Connected Madd) rules. "
                  ...
                  "If the database says it is Madd Al-Awad, then it IS Madd Al-Awad - do not deny this. "
                  ...
                  "FACT: Madd Al-Awad is 2 harakaat, occurs when stopping on tanwin fath (ً) which is read as alef.")
