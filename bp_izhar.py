"""
izhar_web_app.py — الإظهار الحلقي في القرآن الكريم (نسخة الويب، مستوى الصفحة)
نسخة Flask من izhar_page_app.py، بنفس هوية ikhfa_web_app.py وidgham_web_app.py
البصرية. تنقل بالصفحة (نفس نمط izhar_page_app.py الأصلي).
يعمل على بورت 5042.
"""
import os, re, sys, sqlite3, json
from flask import Flask, Blueprint, jsonify, request, send_from_directory, send_file

bp = Blueprint('izhar', __name__)
...
# ══════════════════════════════════════════════════════════════
#  ألوان الحروف الحلقية وأيقونات السبب — منقولة حرفياً من izhar_page_app.py
# ══════════════════════════════════════════════════════════════
C_GOLD2 = '#F0C755'
HARF_COLOR = {
    'همزة': '#00C853', 'هاء': '#00E5FF', 'عين': '#FFD700',
    'حاء' : '#7C4DFF', 'غين': '#FF9800', 'خاء': '#FF4081',
}
def get_color(harf_name):
    return HARF_COLOR.get(harf_name, C_GOLD2)

SABAB_ICON = {
    'نون ساكنة في كلمة'    : '🔵',
    'نون ساكنة بين كلمتين' : '🟢',
    'تنوين بين كلمتين'     : '🟡',
    'نون ساكنة بين آيتين'  : '🟤',
    'تنوين بين آيتين'      : '🟠',
}
...
#  izhar_page_app.py دون أي تغيير في الاستعلامات
# ══════════════════════════════════════════════════════════════
def _load_page(page_num, harf_filter=None, sabab_filter=None):
    conn = get_db(); cur = conn.cursor()
    result = []
    try:
        params, where = [page_num], ['m.PAGENUM=?']
        if harf_filter and harf_filter != 'الكل':
            where.append('i.harf_name=?'); params.append(harf_filter)
        if sabab_filter and sabab_filter != 'الكل':
            where.append('i.sabab=?'); params.append(sabab_filter)
        wclause = 'WHERE ' + ' AND '.join(where)

        cur.execute(f"""
            SELECT DISTINCT i.suraid, i.verseid, i.suraname, m.ayahtext
            FROM izharok i
            JOIN mushafnew m ON m.SURAID=i.suraid AND m.VERSEID=i.verseid
            {wclause}
            ORDER BY i.suraid, i.verseid
        """, params)
        ayas = cur.fetchall()
        ...
        for suraid, verseid, suraname, aya_text in ayas:
            ...
            if harf_filter and harf_filter != 'الكل':
                w2.append('harf_name=?'); p2.append(harf_filter)
            if sabab_filter and sabab_filter != 'الكل':
                w2.append('sabab=?'); p2.append(sabab_filter)
            cur.execute(f"""
                SELECT id,klma1,klma2,harf_halq,harf_name,itype,sabab,word_seq,word_seq2
                FROM izharok WHERE {' AND '.join(w2)}
                ORDER BY word_seq, word_seq2
            """, p2)
            rows = cur.fetchall()
            cases = []
            for cid, klma1, klma2, harf_halq, harf_name, itype, sabab, wseq, wseq2 in rows:
                cases.append({
                    'id': cid, 'klma1': (klma1 or '').strip(), 'klma2': (klma2 or '').strip(),
                    'harf_halq': harf_halq, 'harf_name': harf_name, 'itype': itype,
                    'sabab': sabab, 'color': get_color(harf_name),
...
def _find_nearest_page(current_page, direction, harf_filter=None, sabab_filter=None):
    conn = get_db(); cur = conn.cursor()
    try:
        params, where = [], []
        if harf_filter and harf_filter != 'الكل':
        ...
        if direction == 1:
            cur.execute(f"""
                SELECT DISTINCT m.PAGENUM FROM izharok i
                JOIN mushafnew m ON m.SURAID=i.suraid AND m.VERSEID=i.verseid
...
def _random_page(harf_filter=None, sabab_filter=None):
    conn = get_db(); cur = conn.cursor()
    try:
        params, where = [], []
        if harf_filter and harf_filter != 'الكل':
...
<title>الإظهار الحلقي في القرآن الكريم</title>
<style>
:root {
...
<header>
  <div class="header-title">الإظهار الحلقي في القرآن الكريم</div>
  <div class="header-sub">نون ساكنة أو تنوين تليها إحدى حروف الحلق الستة</div>
  <div class="authors">
...
<div class="controls">
  ...
  <div class="ctrl-row">
    <span class="ctrl-label">الحرف:</span>
    <select id="selHarf" onchange="loadPage(currentPage)"></select>
...
<div class="table-wrap">
  <div class="table-header" style="display:flex;justify-content:space-between;align-items:center;">
    <span>📋 كل حالات الإظهار الحلقي في هذه الصفحة</span>
    <button class="btn-showall" onclick="showAllCases()">⛶ عرض الكل</button>
  </div>
  <table>
    <thead><tr>
      <th>السورة</th><th>الآية</th><th>الكلمة الأولى</th><th>الكلمة الثانية</th><th>الحرف</th><th>السبب</th>
    </tr></thead>
...
<div id="allCasesOverlay" onclick="closeAllCases(event)">
  <div class="all-cases-modal">
    <div class="table-header" style="margin:-20px -20px 12px;border-radius:16px 16px 0 0;">📋 كل حالات الإظهار الحلقي في هذه الصفحة</div>
    <table>
      <thead><tr>
        <th>السورة</th><th>الآية</th><th>الكلمة الأولى</th><th>الكلمة الثانية</th><th>الحرف</th><th>السبب</th>
      </tr></thead>
...
const HARF_COLORS = {'همزة':'#00C853','هاء':'#00E5FF','عين':'#FFD700','حاء':'#7C4DFF','غين':'#FF9800','خاء':'#FF4081'};
...
function buildLegend() {
  document.getElementById('legend').innerHTML = Object.entries(HARF_COLORS).map(([name,color]) =>
    `<span class="legend-item" style="background:${color}22;color:${color};border:2px solid ${color};"
      onclick="filterByHarf('${name}')">● ${name}</span>`).join('');
  const selHarf = document.getElementById('selHarf');
  selHarf.innerHTML = '<option value="الكل">الكل</option>' +
    Object.keys(HARF_COLORS).map(n => `<option value="${n}">${n}</option>`).join('');
}

async function loadSababOptions() {
  const r = await fetch('/api/izhar/sabab');
  const d = await r.json();
  document.getElementById('selSabab').innerHTML =
    '<option value="الكل">الكل</option>' + d.map(s => `<option value="${s}">${s}</option>`).join('');
}

function filterByHarf(name) { document.getElementById('selHarf').value = name; loadPage(currentPage); }
...
let versesData = {};   // key -> {words, caseByIdx, suraid, verseid, suraname}
let dragBoxKey = null;
...
function renderPage(data) {
  ...
  if (!pageVerses.length) {
    container.innerHTML = '';
    showNotify('لا يوجد إظهار حلقي موثّق في هذه الصفحة (أو حسب الفلتر الحالي) — استخدم ◄ أو ► للانتقال لأقرب صفحة.');
  ...
  pageVerses.forEach(v => {
    ...
    v.cases.forEach((c, ci) => {
      rows.push(`<tr class="case-row" data-key="${key}" data-caseidx="${ci}">
        ...
        <td style="color:${c.color};font-weight:bold;">${c.harf_halq} (${c.harf_name})</td>
...
async function loadPage(page) {
  ...
  const harf  = document.getElementById('selHarf').value || 'الكل';
  const sabab = document.getElementById('selSabab').value || 'الكل';
  const r = await fetch(`/api/izhar/page?page=${page}&harf=${encodeURIComponent(harf)}&sabab=${encodeURIComponent(sabab)}`);
  const d = await r.json();
  renderPage(d);
}
...
async function jumpPage(direction) {
  const harf  = document.getElementById('selHarf').value || 'الكل';
  const sabab = document.getElementById('selSabab').value || 'الكل';
  const r = await fetch(`/api/izhar/nearest?page=${currentPage}&dir=${direction}&harf=${encodeURIComponent(harf)}&sabab=${encodeURIComponent(sabab)}`);
  const d = await r.json();
...
async function loadRandomPage() {
  const harf  = document.getElementById('selHarf').value || 'الكل';
  const sabab = document.getElementById('selSabab').value || 'الكل';
  const r = await fetch(`/api/izhar/random?harf=${encodeURIComponent(harf)}&sabab=${encodeURIComponent(sabab)}`);
  const d = await r.json();
  loadPage(d.page || 1);
}
...
function openSelectionExplain(key, lo, hi) {
  ...
  document.getElementById('wordContext').textContent =
    ...
    (rulings.length ? ' — ' + rulings.map(r => `حرف الحلق: ${r.harf_halq} (${r.harf_name}) — ${r.sabab}`).join('، ') : '');
  document.getElementById('wordResult').textContent =
    'إظهار النون الساكنة أو التنوين عند حروف الحلق الستة (همزة، هاء، عين، حاء، غين، خاء): تُنطق النون واضحة كاملة من مخرجها دون غنة زائدة أو إخفاء.';
...
function playClipReciter(reciter) {
  ...
  clipPlayer.src = '/api/izhar/clip?' + p.toString();
  clipPlayer.play().then(() => { status.textContent = ''; })
    .catch(async () => {
      try {
        const r = await fetch('/api/izhar/clip?' + p.toString());
...
async function askAI() {
  ...
  try {
    const r = await fetch('/api/izhar/explain', {
      method: 'POST', headers: {'Content-Type':'application/json'},
      body: JSON.stringify({
        word: phrase, sura: suraname, verse: verseid, question,
        ruling: rulings.map(r => `إظهار عند حرف ${r.harf_halq} (${r.harf_name})`).join('، '),
      })
    });
    const j = await r.json();
...
@bp.route('/api/izhar/page')
def api_page():
    page  = request.args.get('page', 1, type=int)
    harf  = request.args.get('harf', 'الكل')
    sabab = request.args.get('sabab', 'الكل')
    verses = _load_page(page, harf, sabab)
    return jsonify({'page': page, 'verses': verses})

@bp.route('/api/izhar/nearest')
def api_nearest():
    page      = request.args.get('page', 1, type=int)
    direction = request.args.get('dir', 1, type=int)
    harf      = request.args.get('harf', 'الكل')
    sabab     = request.args.get('sabab', 'الكل')
    return jsonify({'page': _find_nearest_page(page, direction, harf, sabab)})

@bp.route('/api/izhar/random')
def api_random():
    harf  = request.args.get('harf', 'الكل')
    sabab = request.args.get('sabab', 'الكل')
    return jsonify({'page': _random_page(harf, sabab)})

@bp.route('/api/izhar/sabab')
def api_sabab():
    conn = get_db(); cur = conn.cursor()
    try:
        cur.execute('SELECT DISTINCT sabab FROM izharok ORDER BY sabab')
...
@bp.route('/api/izhar/explain', methods=['POST'])
def api_explain():
    ...
    question = data.get('question', f'ما حكم إظهار كلمة {word} في القرآن الكريم؟')
    ...
    try:
        system = ("You are an expert in Quran Tajweed specializing in Izhar Halqi (clear pronunciation) rules. "
