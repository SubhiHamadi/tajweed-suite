...
<title>أحكام مد البدل في القرآن الكريم</title>
<style>
:root {
  --navy:  #00BCD4;
  --navy2: #0A2040;
...
<header>
  <div class="header-title">أحكام مد البدل في القرآن الكريم</div>
  <div class="header-sub">إبدال الهمزة الساكنة حرف مدٍّ من جنس حركة ما قبلها — مقداره ثابت حركتان</div>
  <div class="authors">
...
<div class="controls">
  ...
  <div class="ctrl-row" style="justify-content:center;">
    <button class="btn-toggle" onclick="showHelp()" style="background:#FCE4EC;color:#880E4F;border:1.5px solid #F48FB1;padding:8px 24px;">📚 تعريفات مد البدل</button>
...
<div id="allCasesOverlay" onclick="closeAllCases(event)">
  <div id="allCasesModal">
    <div id="allCasesTitle">📋 كل حالات مد البدل في هذه الآية</div>
...
<div class="legend">
  <div class="legend-item leg-lkm" id="leg-مد البدل - ألف" onclick="filterByType('مد البدل - ألف',this)">● بدل - ألف</div>
  <div class="legend-item leg-lkf" id="leg-مد البدل - واو" onclick="filterByType('مد البدل - واو',this)">● بدل - واو</div>
  <div class="legend-item leg-lhm" id="leg-مد البدل - ياء" onclick="filterByType('مد البدل - ياء',this)">● بدل - ياء</div>
</div>

<!-- نافذة التعريفات -->
<div class="help-overlay" id="helpOverlay" onclick="hideHelp(event)">
  <div class="help-modal">
    <div class="help-title">📚 تعريفات مد البدل</div>
    <div style="font-size:13px;color:var(--muted);text-align:center;margin-bottom:12px;padding:8px;background:#FCE4EC;border-radius:8px;">
      مد البدل: أن يتقدّم حرف المد على الهمزة الساكنة عوضاً عن تقدّم الهمزة عليه — ومقداره ثابت حركتان دائماً
    </div>
    <div class="help-tabs">
      <button class="help-tab help-tab-lkm" onclick="showTab('lkm')">بدل - ألف</button>
      <button class="help-tab help-tab-lkf" onclick="showTab('lkf')">بدل - واو</button>
      <button class="help-tab help-tab-lhm" onclick="showTab('lhm')">بدل - ياء</button>
    </div>

    <div class="hcontent show" id="tab-lkm">
      <div class="help-def">
        <b style="color:var(--lkm)">مد البدل بالألف</b><br>
        أصله همزتان: الأولى متحركة والثانية ساكنة، فأُبدلت الثانية ألف مد من جنس حركة الأولى (فتحة).
      </div>
      <div class="help-letters" style="color:var(--lkm)">مقدار المد: حركتان ثابتاً</div>
      <div class="help-example">
        <div class="help-ex-label">مثال:</div>
        ﴿ءَامَنُوا﴾ — أصلها (أَأْمَنُوا) فأُبدلت الهمزة الساكنة ألفاً
      </div>
    </div>

    <div class="hcontent" id="tab-lkf">
      <div class="help-def">
        <b style="color:var(--lkf)">مد البدل بالواو</b><br>
        أُبدلت فيه الهمزة الساكنة واواً من جنس حركة الهمزة الأولى (ضمة).
      </div>
      <div class="help-letters" style="color:var(--lkf)">مقدار المد: حركتان ثابتاً</div>
      <div class="help-example">
        <div class="help-ex-label">مثال:</div>
        ﴿أُوتُوا﴾ — أصلها (أُأْتُوا) فأُبدلت الهمزة الساكنة واواً
      </div>
    </div>

    <div class="hcontent" id="tab-lhm">
      <div class="help-def">
        <b style="color:var(--lhm)">مد البدل بالياء</b><br>
        أُبدلت فيه الهمزة الساكنة ياءً من جنس حركة الهمزة الأولى (كسرة).
      </div>
      <div class="help-letters" style="color:var(--lhm)">مقدار المد: حركتان ثابتاً</div>
      <div class="help-example">
        <div class="help-ex-label">مثال:</div>
        ﴿إِيمَان﴾ — أصلها (إِئْمَان) فأُبدلت الهمزة الساكنة ياءً
      </div>
    </div>
...
<div class="table-wrap">
  <div class="table-header">قائمة حالات مد البدل المستخرجة</div>
  <table>
    <thead>
      <tr>
        <th>ت</th>
...
function showAllCases() {
  if (!currentData || !currentData.cases || !currentData.cases.length) {
    alert('لا يوجد مد بدل موثّق في هذه الآية.');
    return;
  }
...
async function changeAyah(delta) {
  ...
  try {
    const r = await fetch(`/api/badl/nextayah?suraid=${currentData.suraid}&verseid=${currentData.verseid}&delta=${delta}&type=${encodeURIComponent(type)}`);
    const d = await r.json();
    showAyahNavLoading(false);
    if (d.found) {
      const p = {suraid: d.suraid, verseid: d.verseid};
      if (type !== 'الكل') p.type = type;
      const r2 = await fetch('/api/badl/ayah?' + new URLSearchParams(p));
...
async function loadSuras(type) {
  ...
  const r = await fetch(`/api/badl/suras?type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadVerses() {
  ...
  if (!suraid) return;
  const type   = activeFilter || 'الكل';
  const r = await fetch(`/api/badl/verses?suraid=${suraid}&type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadRandom() {
  ...
  const p = {random:1};
  if (type !== 'الكل') p.type = type;
  const r = await fetch('/api/badl/ayah?' + new URLSearchParams(p));
  const d = await r.json();
  if (d.error) return;
  currentData = d;
  renderAyah(d);
...
function renderAyah(d) {
  ...
  // العدادات
  const counts = {'مد البدل - ألف':0,'مد البدل - واو':0,'مد البدل - ياء':0};
  ...
  document.getElementById('infoCounts').innerHTML = Object.entries(counts)
    .filter(([,n])=>n>0)
    .map(([k,n])=>`<span class="cnt-badge" style="${styleMap[k]}">${k.replace('مد البدل - ','')}: ${n}</span>`)
    .join('');
...
function playClipReciter(reciter) {
  ...
  clipPlayer.src = '/api/badl/clip?' + p.toString();
  clipPlayer.play().then(() => { status.textContent = ''; })
    .catch(async () => {
      try {
        const r = await fetch('/api/badl/clip?' + p.toString());
        const d = await r.json();
...
const WORD_QUESTIONS = {
  1: 'ما حكم هذه الكلمة من أحكام مد البدل؟',
...
async function askWordQuestion(num) {
  ...
  try {
    const r = await fetch('/api/badl/explain', {
...
def api_ayah():
    ...
    try:
        cur.execute('''SELECT suraname, ayahtext, pagenum
                       FROM md_badl WHERE suraid=? AND verseid=? LIMIT 1''',
                    (suraid, verseid))
...
@bp.route('/api/badl/explain', methods=['POST'])
def api_explain():
    ...
    try:
        system = ("You are an expert in Quran Tajweed specializing in Madd rules. "
                  ...
                  "If the database says it is Madd Al-Badl, then it IS Madd Al-Badl - do not deny this. "
                  ...
                  "FACT: Madd Al-Badl is always exactly 2 harakaat (a fixed short madd), occurring "
                  "when a madd letter (alif/waw/ya) replaces an original saakin hamza.")
...
def api_clip():
    ...
    conn = get_db(); cur = conn.cursor()
    cur.execute('SELECT ayahtext FROM md_badl WHERE suraid=? AND verseid=? LIMIT 1', (suraid, verseid))
