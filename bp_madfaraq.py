"""
mad_faraq_web_app.py — أحكام مد الفرق في القرآن الكريم
مد الفرق محصور حصراً في 6 مواضع عبر القرآن كله (حفص عن عاصم) — وليس
حكماً منتشراً كبقية أحكام المدود، لذا البيانات هنا ثابتة ومضمّنة مباشرة
(لا يوجد جدول قاعدة بيانات مخصص لهذا الحكم).
يعمل على بورت 5029
"""
import os, re, sys, sqlite3, json
from flask import Flask, Blueprint, jsonify, request, send_from_directory, send_file

bp = Blueprint('madfaraq', __name__)
import sys
...
ITYPE_COLORS = {
    'فرق - ءَآلذَّكَرَيْنِ' : '#F0C755',
    'فرق - ءَآللَّهُ'       : '#00E5FF',
    'فرق - ءَآلْـَٰٔنَ'      : '#00C853',
}

# المواضع الستة الوحيدة لمد الفرق في القرآن الكريم (رواية حفص عن عاصم)
FARQ_DATA = [
    {'suraid': 6,  'verseid': 143, 'word': 'ءَآلذَّكَرَيْنِ', 'itype': 'فرق - ءَآلذَّكَرَيْنِ',
     'desc': 'مد فرق بسبب دخول همزة الاستفهام على لام التعريف المدمجة بالذال'},
    {'suraid': 6,  'verseid': 144, 'word': 'ءَآلذَّكَرَيْنِ', 'itype': 'فرق - ءَآلذَّكَرَيْنِ',
     'desc': 'الموضع الثاني لمد الفرق في سورة الأنعام'},
    {'suraid': 10, 'verseid': 59,  'word': 'ءَآللَّهُ',       'itype': 'فرق - ءَآللَّهُ',
     'desc': 'مد استفهام مشبع للتفريق بين الخبر والاستخبار في لفظ الجلالة'},
    {'suraid': 27, 'verseid': 59,  'word': 'ءَآللَّهُ',       'itype': 'فرق - ءَآللَّهُ',
     'desc': 'الموضع الثاني بلفظ الجلالة في سورة النمل'},
    {'suraid': 10, 'verseid': 51,  'word': 'ءَآلْـَٰٔنَ',      'itype': 'فرق - ءَآلْـَٰٔنَ',
     'desc': 'مد الفرق بدخول همزة الاستفهام على اللام الساكنة المظهرة'},
    {'suraid': 10, 'verseid': 91,  'word': 'ءَآلْـَٰٔنَ',      'itype': 'فرق - ءَآلْـَٰٔنَ',
     'desc': 'الموضع الثاني في قصة فرعون عند إدراكه الغرق'},
]
FARQ_LENGTH_NOTE = '6'
...
# ── مطابقة هيكلية مرنة للكلمة (بدل التساوي الحرفي الصارم) ──────────
# سبب الحاجة إليها: كلمات FARQ_DATA مكتوبة يدوياً بالإملاء القياسي، بينما
# نص mushafnew.ayahtext هو الرسم العثماني للمصحف — قد يختلفان في تفاصيل
# دقيقة من التشكيل (كالألف الخنجرية، ترتيب الشدة/الحركة) رغم كونهما نفس
# الكلمة تماماً. بلا هذه المرونة، تفشل المطابقة الحرفية فيسقط الموضع
# افتراضياً وخطأً إلى أول كلمة في الآية (راجع _find_word_position أدناه).
...
def _skeleton(text):
    # تطبيع NFD أولاً: يفكّك الحروف المُدمَجة (كالألف الممدودة آ = ا+ٓ،
    # والهمزات على الألف/الواو/الياء) إلى حرفها الأساسي + علامة الهمزة/المدة
    # كعلامة تشكيل منفصلة — فتُزال لاحقاً كأي تشكيل آخر، مهما كان الرمز
    # الأصلي مُدمَجاً (رمز واحد) أو مفكَّكاً (رمزين) في مصدر النص.
    text = _ud.normalize('NFD', text)
    for special, standard in _QURANIC_ORTHOGRAPHY_MAP.items():
        text = text.replace(special, standard)
    return ''.join(ch for ch in text if _ud.category(ch) != 'Mn')

def _find_word_position(aya_words, target_word):
    """يُرجع فهرس الكلمة (1-based) داخل aya_words، أو None إن تعذّر
    إيجادها فعلياً بأي مستوى مرونة — لا افتراض صامت لموضع خاطئ أبداً."""
    w = target_word.strip()
    for i, ww in enumerate(aya_words):
        if ww.strip() == w:
            return i + 1
    w_skel = _skeleton(w)
    if w_skel:
        for i, ww in enumerate(aya_words):
            if _skeleton(ww.strip()) == w_skel:
                return i + 1
    return None
...
<title>أحكام مد الفرق في القرآن الكريم</title>
<style>
:root {
  --navy:  #00BCD4;
  --navy2: #0A2040;
...
<header>
  <div class="header-title">أحكام مد الفرق في القرآن الكريم</div>
  <div class="header-sub">إبدال همزة الوصل حرف مدٍّ بعد دخول همزة الاستفهام — 6 مواضع فقط في القرآن كله</div>
  <div class="authors">
...
<div class="controls">
  ...
  <div class="ctrl-row" style="justify-content:center;">
    <button class="btn-toggle" onclick="showHelp()" style="background:#FCE4EC;color:#880E4F;border:1.5px solid #F48FB1;padding:8px 24px;">📚 تعريفات مد الفرق</button>
...
<div id="allCasesOverlay" onclick="closeAllCases(event)">
  <div id="allCasesModal">
    <div id="allCasesTitle">📋 كل حالات مد الفرق في هذه الآية</div>
...
<div class="legend">
  <div class="legend-item leg-lkm" id="leg-فرق - ءَآلذَّكَرَيْنِ" onclick="filterByType('فرق - ءَآلذَّكَرَيْنِ',this)">● ءَآلذَّكَرَيْنِ</div>
  <div class="legend-item leg-lkf" id="leg-فرق - ءَآللَّهُ" onclick="filterByType('فرق - ءَآللَّهُ',this)">● ءَآللَّهُ</div>
  <div class="legend-item leg-lhm" id="leg-فرق - ءَآلْـَٰٔنَ" onclick="filterByType('فرق - ءَآلْـَٰٔنَ',this)">● ءَآلْـَٰٔنَ</div>
</div>

<!-- نافذة التعريفات -->
<div class="help-overlay" id="helpOverlay" onclick="hideHelp(event)">
  <div class="help-modal">
    <div class="help-title">📚 تعريف مد الفرق</div>
    <div style="font-size:13px;color:var(--muted);text-align:center;margin-bottom:12px;padding:8px;background:#FCE4EC;border-radius:8px;">
      مد الفرق: أن تدخل همزة الاستفهام على اسم معرَّف بـ"ال"، فتُبدل همزة الوصل حرف مد مقداره 6 حركات؛
      للتفريق بين أسلوب الاستفهام والخبر. ورد في 3 كلمات فقط، في 6 مواضع حصراً عبر القرآن كله.
    </div>
    <div class="help-tabs">
      <button class="help-tab help-tab-lkm" onclick="showTab('lkm')">ءَآلذَّكَرَيْنِ</button>
      <button class="help-tab help-tab-lkf" onclick="showTab('lkf')">ءَآللَّهُ</button>
      <button class="help-tab help-tab-lhm" onclick="showTab('lhm')">ءَآلْـَٰٔنَ</button>
    </div>

    <div class="hcontent show" id="tab-lkm">
      <div class="help-def">
        <b style="color:var(--lkm)">ءَآلذَّكَرَيْنِ</b><br>
        دخلت همزة الاستفهام على "الذَّكَرَيْنِ" (المعرَّفة بلام شمسية مدغمة في الذال)، فأُبدلت همزة الوصل
        ألفاً مشبعة تفريقاً بين الاستفهام والخبر.
      </div>
      <div class="help-letters" style="color:var(--lkm)">مقدار المد: 6 حركات وجوباً</div>
      <div class="help-example">
        <div class="help-ex-label">موضعان:</div>
        سورة الأنعام، الآيتان 143 و144
      </div>
    </div>

    <div class="hcontent" id="tab-lkf">
      <div class="help-def">
        <b style="color:var(--lkf)">ءَآللَّهُ</b><br>
        دخلت همزة الاستفهام على لفظ الجلالة "اللَّهُ"، فأُبدلت همزة الوصل ألفاً مشبعة للتمييز بين
        الاستخبار والتعظيم.
      </div>
      <div class="help-letters" style="color:var(--lkf)">مقدار المد: 6 حركات وجوباً</div>
      <div class="help-example">
        <div class="help-ex-label">موضعان:</div>
        سورة يونس الآية 59 &nbsp;·&nbsp; سورة النمل الآية 59
      </div>
    </div>

    <div class="hcontent" id="tab-lhm">
      <div class="help-def">
        <b style="color:var(--lhm)">ءَآلْـَٰٔنَ</b><br>
        دخلت همزة الاستفهام على "الآنَ" (اللام الساكنة مظهرة)، فأُبدلت همزة الوصل ألفاً مشبعة.
      </div>
      <div class="help-letters" style="color:var(--lhm)">مقدار المد: 6 حركات وجوباً</div>
      <div class="help-example">
        <div class="help-ex-label">موضعان (كلاهما في قصة فرعون):</div>
        سورة يونس، الآيتان 51 و91
      </div>
    </div>
...
<div class="table-wrap">
  <div class="table-header">قائمة حالات مد الفرق المستخرجة</div>
  <table>
    <thead>
      <tr>
        <th>ت</th>
...
function showAllCases() {
  if (!currentData || !currentData.cases || !currentData.cases.length) {
    alert('لا يوجد مد فرق موثّق في هذه الآية.');
    return;
  }
...
async function changeAyah(delta) {
  ...
  try {
    const r = await fetch(`/api/faraq/nextayah?suraid=${currentData.suraid}&verseid=${currentData.verseid}&delta=${delta}&type=${encodeURIComponent(type)}`);
    const d = await r.json();
    showAyahNavLoading(false);
    if (d.found) {
      const p = {suraid: d.suraid, verseid: d.verseid};
      if (type !== 'الكل') p.type = type;
      const r2 = await fetch('/api/faraq/ayah?' + new URLSearchParams(p));
...
async function loadSuras(type) {
  ...
  const r = await fetch(`/api/faraq/suras?type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadVerses() {
  ...
  if (!suraid) return;
  const type   = activeFilter || 'الكل';
  const r = await fetch(`/api/faraq/verses?suraid=${suraid}&type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadRandom() {
  ...
  const p = {random:1};
  if (type !== 'الكل') p.type = type;
  const r = await fetch('/api/faraq/ayah?' + new URLSearchParams(p));
  const d = await r.json();
  if (d.error) return;
  currentData = d;
  renderAyah(d);
...
function renderAyah(d) {
  ...
  // العدادات
  const counts = {'فرق - ءَآلذَّكَرَيْنِ':0,'فرق - ءَآللَّهُ':0,'فرق - ءَآلْـَٰٔنَ':0};
  d.cases.forEach(c => { if (counts[c.itype] !== undefined) counts[c.itype]++; });
  const styleMap = {
    'فرق - ءَآلذَّكَرَيْنِ': 'background:#FFF8E1;color:#F0C755;border:1px solid #F0C755',
    'فرق - ءَآللَّهُ': 'background:#E0F7FA;color:#00E5FF;border:1px solid #00E5FF',
    'فرق - ءَآلْـَٰٔنَ': 'background:#E8F5E9;color:#00C853;border:1px solid #00C853',
  };
  document.getElementById('infoCounts').innerHTML = Object.entries(counts)
    .filter(([,n])=>n>0)
    .map(([k,n])=>`<span class="cnt-badge" style="${styleMap[k]}">${k.replace('فرق - ','')}: ${n}</span>`)
    .join('');
  ...
  d.cases.forEach(c => {
    ...
    // نثق مباشرة بالموضع الذي حسبه الخادم (مطابقة هيكلية مرنة) — إعادة
    // تحقق حرفية صارمة هنا كانت تُبطل الإصلاح لنفس سبب فرق الرسم/التشكيل.
...
function playClipReciter(reciter) {
  ...
  clipPlayer.src = '/api/faraq/clip?' + p.toString();
  clipPlayer.play().then(() => { status.textContent = ''; })
    .catch(async () => {
      try {
        const r = await fetch('/api/faraq/clip?' + p.toString());
        const d = await r.json();
...
const WORD_QUESTIONS = {
  1: 'ما حكم هذه الكلمة من أحكام مد الفرق؟',
...
async function askWordQuestion(num) {
  ...
  try {
    const r = await fetch('/api/faraq/explain', {
...
def api_nextayah():
    ...
    found   = None
    try:
        items = [d for d in FARQ_DATA if (not itype or itype == 'الكل' or d['itype'] == itype)]
        all_ayat = sorted({(d['suraid'], d['verseid']) for d in items})
        cur_idx = None
        for i, (s, v) in enumerate(all_ayat):
            if s == suraid and v == verseid:
                cur_idx = i; break
        if cur_idx is None:
            for i, (s, v) in enumerate(all_ayat):
                if (s > suraid) or (s == suraid and v > verseid):
...
@bp.route('/api/faraq/suras')
def api_suras():
    itype = request.args.get('type','الكل')
    conn = get_db(); cur = conn.cursor()
    items = [d for d in FARQ_DATA if (not itype or itype == 'الكل' or d['itype'] == itype)]
    suraids = sorted({d['suraid'] for d in items})
    rows = []
    for sid in suraids:
        try:
            cur.execute('SELECT SURANAME FROM mushafnew WHERE SURAID=? LIMIT 1', (sid,))
            r = cur.fetchone()
            rows.append({'suraid': sid, 'suraname': r[0] if r else ''})
        except: rows.append({'suraid': sid, 'suraname': ''})
    conn.close(); return jsonify(rows)


@bp.route('/api/faraq/verses')
def api_verses():
    suraid = request.args.get('suraid')
    itype  = request.args.get('type','الكل')
    if not suraid: return jsonify([])
    suraid = int(suraid)
    items = [d for d in FARQ_DATA if d['suraid'] == suraid and (not itype or itype == 'الكل' or d['itype'] == itype)]
    return jsonify(sorted({d['verseid'] for d in items}))


@bp.route('/api/faraq/ayah')
def api_ayah():
    suraid  = request.args.get('suraid')
    ...
    if rand == '1' and items:
        ...
        suraid, verseid = pick['suraid'], pick['verseid']
    elif suraid: suraid = int(suraid)
    if verseid: verseid = int(verseid)

    if not suraid or not verseid:
        conn.close(); return jsonify({'error':'missing'})

    # جلب حالات مد الفرق لهذه الآية (من القائمة الثابتة المضمَّنة، وليس من قاعدة البيانات)
    matches = [d for d in FARQ_DATA if d['suraid'] == suraid and d['verseid'] == verseid]

    # جلب نص الآية
    try:
        cur.execute('''SELECT SURANAME, ayahtext, PAGENUM
                       FROM mushafnew WHERE SURAID=? AND VERSEID=? LIMIT 1''',
                    (suraid, verseid))
        meta = cur.fetchone() or ('','','')
    except: meta = ('','','')

    aya_clean = _clean_aya(meta[1])
    aya_words = aya_clean.split(' ') if aya_clean else []

    cases = []
    for d in matches:
        position = _find_word_position(aya_words, d['word'])
        if position is None:
            position = 1  # احتياط أخير فقط (لن يحدث عملياً بعد إضافة المطابقة الهيكلية)
        cases.append({
            'word': d['word'], 'itype': d['itype'], 'length': FARQ_LENGTH_NOTE,
            'position': position, 'color': get_color(d['itype']),
        })
...
@bp.route('/api/faraq/explain', methods=['POST'])
def api_explain():
    ...
    try:
        system = ("You are an expert in Quran Tajweed specializing in Madd rules. "
                  ...
                  "If the database says it is Madd Al-Farq, then it IS Madd Al-Farq - do not deny this. "
                  ...
                  "FACT: Madd Al-Farq occurs only in exactly 6 fixed positions across the entire "
                  "Quran (Hafs 'an 'Asim): three words repeated twice each. Its length is always "
                  "6 harakaat, and it results from replacing the hamzat al-wasl with a madd letter "
                  "after an interrogative hamza, to distinguish a question from a statement.")
