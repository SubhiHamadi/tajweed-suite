...
# مد اللين له حكم واحد (لا أنواع فرعية)، وطوله يتراوح بين 2 أو 4 أو 6
# حركات حسب حالة الوقف/الوصل واختيار القارئ
ITYPE_COLORS = {
    'مد اللين' : '#69F0AE',
}
LEEN_LENGTH_NOTE = '2 أو 4 أو 6'
...
<title>أحكام مد اللين في القرآن الكريم</title>
<style>
:root {
  --navy:  #00BCD4;
  --navy2: #0A2040;
...
<header>
  <div class="header-title">أحكام مد اللين في القرآن الكريم</div>
  <div class="header-sub">إطالة الصوت في حرف الواو أو الياء الساكنَين المفتوح ما قبلهما</div>
  <div class="authors">
...
<div class="controls">
  ...
  <div class="ctrl-row" style="justify-content:center;">
    <button class="btn-toggle" onclick="showHelp()" style="background:#FCE4EC;color:#880E4F;border:1.5px solid #F48FB1;padding:8px 24px;">📚 تعريفات مد اللين</button>
...
<div id="allCasesOverlay" onclick="closeAllCases(event)">
  <div id="allCasesModal">
    <div id="allCasesTitle">📋 كل حالات مد اللين في هذه الآية</div>
...
<div class="legend">
  <div class="legend-item leg-lkm" id="leg-مد اللين" onclick="filterByType('مد اللين',this)">● مد اللين</div>
</div>

<!-- نافذة التعريفات -->
<div class="help-overlay" id="helpOverlay" onclick="hideHelp(event)">
  <div class="help-modal">
    <div class="help-title">📚 تعريف مد اللين</div>
    <div style="font-size:13px;color:var(--muted);text-align:center;margin-bottom:12px;padding:8px;background:#FCE4EC;border-radius:8px;">
      مد اللين: أن يأتي حرف الواو أو الياء الساكنَين (المفتوح ما قبلهما) قبل حرفٍ ساكن سكوناً عارضاً بسبب الوقف.
    </div>

    <div class="hcontent show" id="tab-lkm">
      <div class="help-def">
        <b style="color:var(--lkm)">مد اللين</b><br>
        يقع حين تسبق الواو الساكنة أو الياء الساكنة فتحةٌ، ثم يُوقف على حرف بعدهما فيصبح ساكناً سكوناً عارضاً.
        يزول حكم المد إذا وُصلت القراءة ولم يُوقف على الكلمة.
      </div>
...
<div class="table-wrap">
  <div class="table-header">قائمة حالات مد اللين المستخرجة</div>
  <table>
    <thead>
      <tr>
        <th>ت</th>
...
function showAllCases() {
  if (!currentData || !currentData.cases || !currentData.cases.length) {
    alert('لا يوجد مد لين موثّق في هذه الآية.');
    return;
  }
...
async function changeAyah(delta) {
  ...
  try {
    const r = await fetch(`/api/leen/nextayah?suraid=${currentData.suraid}&verseid=${currentData.verseid}&delta=${delta}&type=${encodeURIComponent(type)}`);
    const d = await r.json();
    showAyahNavLoading(false);
    if (d.found) {
      const p = {suraid: d.suraid, verseid: d.verseid};
      if (type !== 'الكل') p.type = type;
      const r2 = await fetch('/api/leen/ayah?' + new URLSearchParams(p));
...
async function loadSuras(type) {
  ...
  const r = await fetch(`/api/leen/suras?type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadVerses() {
  ...
  if (!suraid) return;
  const type   = activeFilter || 'الكل';
  const r = await fetch(`/api/leen/verses?suraid=${suraid}&type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadRandom() {
  ...
  const p = {random:1};
  if (type !== 'الكل') p.type = type;
  const r = await fetch('/api/leen/ayah?' + new URLSearchParams(p));
  const d = await r.json();
  if (d.error) return;
  currentData = d;
  renderAyah(d);
...
function renderAyah(d) {
  ...
  // العدادات
  const counts = {'مد اللين':0};
  d.cases.forEach(c => { if (counts[c.itype] !== undefined) counts[c.itype]++; });
  const styleMap = {
    'مد اللين': 'background:#E0F7EE;color:#00BFA5;border:1px solid #00BFA5',
  };
...
function playClipReciter(reciter) {
  ...
  clipPlayer.src = '/api/leen/clip?' + p.toString();
  clipPlayer.play().then(() => { status.textContent = ''; })
    .catch(async () => {
      try {
        const r = await fetch('/api/leen/clip?' + p.toString());
        const d = await r.json();
...
const WORD_QUESTIONS = {
  1: 'ما حكم هذه الكلمة من أحكام مد اللين؟',
...
async function askWordQuestion(num) {
  ...
  try {
    const r = await fetch('/api/leen/explain', {
...
def api_nextayah():
    ...
    found   = None
    try:
        cur.execute('''SELECT DISTINCT suraid, verseid FROM leeen
...
@bp.route('/api/leen/suras')
def api_suras():
    conn = get_db(); cur = conn.cursor()
    try:
        cur.execute('''SELECT DISTINCT suraid, suraname FROM leeen
...
@bp.route('/api/leen/verses')
def api_verses():
    suraid = request.args.get('suraid')
    if not suraid: return jsonify([])
    conn = get_db(); cur = conn.cursor()
    try:
        cur.execute('''SELECT DISTINCT verseid FROM leeen
...
def api_ayah():
    ...
    if rand == '1':
        try:
            cur.execute('SELECT DISTINCT suraid,verseid FROM leeen ORDER BY RANDOM() LIMIT 1')
            r = cur.fetchone()
    ...
    # جلب حالات مد اللين في هذه الآية (klmahr=الكلمة، kseq=موضعها 1-based)
    try:
        cur.execute('''SELECT klmahr, kseq FROM leeen
                       WHERE suraid=? AND verseid=? ORDER BY kseq''', (suraid, verseid))
    ...
    cases = [{
        'word'    : c[0],
        'itype'   : 'مد اللين',
        'length'  : LEEN_LENGTH_NOTE,
        'position': c[1],
        'color'   : get_color('مد اللين')
    } for c in cases_raw]
...
@bp.route('/api/leen/explain', methods=['POST'])
def api_explain():
    ...
    try:
        system = ("You are an expert in Quran Tajweed specializing in Madd rules. "
                  ...
                  "If the database says it is Madd Leen (مد اللين), then it IS that ruling - do not deny this. "
                  ...
                  "FACT: Madd Leen occurs when a saakin waw or yaa preceded by a fatha is followed by "
                  "a letter that becomes saakin due to pausing (waqf); it may be read as 2, 4, or 6 harakaat, "
                  "the reciter's choice, and it disappears in continuous recitation (wasl).")
