...
ITYPE_COLORS = {
    'المد الواجب المتصل - ألف' : '#E040FB',
    'المد الواجب المتصل - واو' : '#7C4DFF',
    'المد الواجب المتصل - ياء' : '#40C4FF',
    'مد متصل في الياء'         : '#69F0AE',
}
...
<title>أحكام المد المتصل في القرآن الكريم</title>
<style>
:root {
  --navy:  #00BCD4;
  --navy2: #0A2040;
...
<header>
  <div class="header-title">أحكام المد المتصل في القرآن الكريم</div>
  <div class="header-sub">المد الواجب المتصل — مد 4 أو 5 حركات عند الهمز</div>
  <div class="authors">
...
<div class="controls">
  ...
  <div class="ctrl-row" style="justify-content:center;">
    <button class="btn-toggle" onclick="showHelp()" style="background:#EDE7F6;color:#512DA8;border:1.5px solid #9575CD;padding:8px 24px;">📚 تعريفات المد المتصل</button>
...
<div id="allCasesOverlay" onclick="closeAllCases(event)">
  <div id="allCasesModal">
    <div id="allCasesTitle">📋 كل حالات المد المتصل في هذه الآية</div>
    <div id="allCasesCount"></div>
    <table>
      <thead>
        <tr>
          <th>ت</th><th>الكلمة</th><th>نوع المد</th><th>نوع الهمزة</th><th>الموضع</th>
        </tr>
      </thead>
...
<div class="legend">
  <div class="legend-item" id="leg-المد الواجب المتصل - ألف" style="background:rgba(224,64,251,0.1);color:#E040FB;border:2px solid #E040FB" onclick="filterByType('المد الواجب المتصل - ألف',this)">● متصل - ألف</div>
  <div class="legend-item" id="leg-المد الواجب المتصل - واو" style="background:rgba(124,77,255,0.1);color:#7C4DFF;border:2px solid #7C4DFF" onclick="filterByType('المد الواجب المتصل - واو',this)">● متصل - واو</div>
  <div class="legend-item" id="leg-المد الواجب المتصل - ياء" style="background:rgba(64,196,255,0.1);color:#40C4FF;border:2px solid #40C4FF" onclick="filterByType('المد الواجب المتصل - ياء',this)">● متصل - ياء</div>
  <div class="legend-item" id="leg-مد متصل في الياء" style="background:rgba(105,240,174,0.1);color:#69F0AE;border:2px solid #69F0AE" onclick="filterByType('مد متصل في الياء',this)">● متصل في الياء</div>
</div>

<!-- نافذة التعريفات -->
<div class="help-overlay" id="helpOverlay" onclick="hideHelp(event)">
  <div class="help-modal">
    <div class="help-title">📚 تعريفات المد المتصل</div>
    <div style="font-size:13px;color:var(--muted);text-align:center;margin-bottom:12px;padding:8px;background:#EDE7F6;border-radius:8px;">
      المد المتصل (الواجب): حرف مد في كلمة تليه همزة في نفس الكلمة — يُمدّ 4 أو 5 حركات وجوباً
    </div>
    <div class="help-tabs">
      <button class="help-tab" style="border-color:#E040FB;color:#E040FB" onclick="showTab('con-alef')">ألف</button>
      <button class="help-tab" style="border-color:#7C4DFF;color:#7C4DFF" onclick="showTab('con-waw')">واو</button>
      <button class="help-tab" style="border-color:#40C4FF;color:#40C4FF" onclick="showTab('con-ya')">ياء</button>
      <button class="help-tab" style="border-color:#69F0AE;color:#69F0AE" onclick="showTab('con-ya2')">متصل في الياء</button>
    </div>

    <div class="hcontent show" id="tab-con-alef">
      <div class="help-def">
        <b style="color:#E040FB">المد الواجب المتصل — ألف</b><br>
        مدّ الألف (أو ما يقوم مقامها) عند وجود همزة بعدها في نفس الكلمة.
      </div>
      <div class="help-letters" style="color:#E040FB">مقدار المد: 4 أو 5 حركات وجوباً</div>
      <div class="help-example">
        <div class="help-ex-label">أمثلة:</div>
        ﴿جَآءَ﴾ &nbsp;·&nbsp; ﴿السَّمَآءَ﴾ &nbsp;·&nbsp; ﴿شَآءَ﴾
      </div>
    </div>

    <div class="hcontent" id="tab-con-waw">
      <div class="help-def">
        <b style="color:#7C4DFF">المد الواجب المتصل — واو</b><br>
        مدّ واو المد عند وجود همزة بعدها في نفس الكلمة.
      </div>
      <div class="help-letters" style="color:#7C4DFF">مقدار المد: 4 أو 5 حركات وجوباً</div>
      <div class="help-example">
        <div class="help-ex-label">أمثلة:</div>
        ﴿سُوءَ﴾ &nbsp;·&nbsp; ﴿يَسُوءُ﴾
      </div>
    ...
    <div class="hcontent" id="tab-con-ya">
      <div class="help-def">
        <b style="color:#40C4FF">المد الواجب المتصل — ياء</b><br>
        مدّ ياء المد عند وجود همزة بعدها في نفس الكلمة.
      </div>
      <div class="help-letters" style="color:#40C4FF">مقدار المد: 4 أو 5 حركات وجوباً</div>
      <div class="help-example">
        <div class="help-ex-label">أمثلة:</div>
        ﴿الْخَاسِئِينَ﴾ &nbsp;·&nbsp; ﴿جِيءَ﴾
      </div>
    ...
    <div class="hcontent" id="tab-con-ya2">
      <div class="help-def">
        <b style="color:#69F0AE">مد متصل في الياء</b><br>
        حالة خاصة من المد المتصل تقع في بعض الكلمات التي تحتوي ياء المد قبل الهمزة.
      </div>
...
<div class="table-wrap">
  <div class="table-header">قائمة حالات المد المتصل المستخرجة</div>
  <table>
    <thead>
      <tr>
        <th>ت</th>
...
function showAllCases() {
  if (!currentData || !currentData.cases || !currentData.cases.length) {
    alert('لا يوجد مد متصل موثّق في هذه الآية.');
    return;
  }
...
async function changeAyah(delta) {
  ...
  try {
    const r = await fetch(`/api/mdcon/nextayah?suraid=${currentData.suraid}&verseid=${currentData.verseid}&delta=${delta}&type=${encodeURIComponent(type)}`);
    const d = await r.json();
    showAyahNavLoading(false);
    if (d.found) {
      const p = {suraid: d.suraid, verseid: d.verseid};
      if (type !== 'الكل') p.type = type;
      const r2 = await fetch('/api/mdcon/ayah?' + new URLSearchParams(p));
...
async function loadSuras(type) {
  ...
  const r = await fetch(`/api/mdcon/suras?type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadVerses() {
  ...
  if (!suraid) return;
  const type   = activeFilter || 'الكل';
  const r = await fetch(`/api/mdcon/verses?suraid=${suraid}&type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadRandom() {
  ...
  const p = {random:1};
  if (type !== 'الكل') p.type = type;
  const r = await fetch('/api/mdcon/ayah?' + new URLSearchParams(p));
  const d = await r.json();
  if (d.error) return;
  currentData = d;
  renderAyah(d);
...
function renderAyah(d) {
  ...
  // العدادات
  const counts = {'المد الواجب المتصل - ألف':0,'المد الواجب المتصل - واو':0,'المد الواجب المتصل - ياء':0,'مد متصل في الياء':0};
  d.cases.forEach(c => { if (counts[c.itype] !== undefined) counts[c.itype]++; });
  const styleMap = {
    'المد الواجب المتصل - ألف': 'background:#F3E5F5;color:#E040FB;border:1px solid #E040FB',
    'المد الواجب المتصل - واو': 'background:#EDE7F6;color:#7C4DFF;border:1px solid #7C4DFF',
    'المد الواجب المتصل - ياء': 'background:#E1F5FE;color:#40C4FF;border:1px solid #40C4FF',
    'مد متصل في الياء':         'background:#E8F5E9;color:#69F0AE;border:1px solid #69F0AE',
  };
...
function playClipReciter(reciter) {
  ...
  clipPlayer.src = '/api/mdcon/clip?' + p.toString();
  clipPlayer.play().then(() => { status.textContent = ''; })
    .catch(async () => {
      try {
        const r = await fetch('/api/mdcon/clip?' + p.toString());
        const d = await r.json();
...
const WORD_QUESTIONS = {
  1: 'ما حكم هذه الكلمة من أحكام المد المتصل؟',
...
function openWordExplain(word, sura, verse, color, idx) {
  ...
  // نفضّل دائماً الاعتماد على فهرس الكلمة (موضعها الرقمي ضمن الآية) بدل
  // البحث النصي عنها، لأن البحث النصي قد يلتقط أول ظهور لكلمة متكررة
  // في الآية (مثل "مِن") بدل الكلمة المقصودة فعلياً — الفهرس دقيق دائماً.
...
async function askWordQuestion(num) {
  ...
  try {
    const r = await fetch('/api/mdcon/explain', {
...
def api_nextayah():
    ...
    try:
        ...
        if itype and itype != 'الكل':
            cur.execute('''SELECT DISTINCT suraid, verseid FROM md_con
                           WHERE rem1=?
...
@bp.route('/api/mdcon/suras')
def api_suras():
    itype = request.args.get('type','الكل')
    conn = get_db(); cur = conn.cursor()
    try:
        if itype and itype != 'الكل':
            cur.execute('''SELECT DISTINCT c.suraid, c.suraname
                           FROM md_con c
                           WHERE c.rem1=?
                           ORDER BY CAST(c.suraid AS INTEGER)''', (itype,))
        else:
            cur.execute('''SELECT DISTINCT c.suraid, c.suraname
                           FROM md_con c
                           ORDER BY CAST(c.suraid AS INTEGER)''')
        rows = [{'suraid':r[0],'suraname':r[1]} for r in cur.fetchall()]
    except: rows = []
    conn.close(); return jsonify(rows)


@bp.route('/api/mdcon/verses')
def api_verses():
    ...
    try:
        if itype and itype != 'الكل':
            cur.execute('''SELECT DISTINCT verseid FROM md_con
                           WHERE suraid=? AND rem1=?
                           ORDER BY CAST(verseid AS INTEGER)''', (suraid, itype))
        else:
            cur.execute('''SELECT DISTINCT verseid FROM md_con
                           WHERE suraid=?
                           ORDER BY CAST(verseid AS INTEGER)''', (suraid,))
        rows = cur.fetchall()
    except: rows = []
    conn.close(); return jsonify([r[0] for r in rows])


@bp.route('/api/mdcon/ayah')
def api_ayah():
    suraid  = request.args.get('suraid')
    ...
    if rand == '1':
        try:
            if itype and itype != 'الكل':
                cur.execute('SELECT DISTINCT suraid,verseid FROM md_con WHERE rem1=? ORDER BY RANDOM() LIMIT 1', (itype,))
            else:
                cur.execute('SELECT DISTINCT suraid,verseid FROM md_con ORDER BY RANDOM() LIMIT 1')
            r = cur.fetchone()
    ...
    # جلب نص الآية
    try:
        cur.execute('''SELECT c.suraname, c.ayahtext, c.pagenum
                       FROM md_con c WHERE c.suraid=? AND c.verseid=? LIMIT 1''',
                    (suraid, verseid))
...
@bp.route('/api/mdcon/explain', methods=['POST'])
def api_explain():
    ...
    try:
        system = ("You are an expert in Quran Tajweed specializing in Madd Al-Muttasil (Connected Madd) rules. "
                  "Answer briefly in max 3-4 lines. "
                  "CRITICAL: The ruling provided from the database is CORRECT - never contradict it. "
                  "If the database says it is Madd Al-Muttasil, then it IS Madd Al-Muttasil - do not deny this. "
                  ...
                  "FACT: Madd Al-Muttasil is 4 or 5 harakaat depending on the reciter.")
