...
# مد العارض للسكون له حكم واحد (وليس أنواعاً فرعية كالمد اللازم)، لكن
# طوله يتراوح بين 2 أو 4 أو 6 حركات حسب اختيار القارئ
ITYPE_COLORS = {
    'مد عارض للسكون' : '#FF9100',
}
ARTH_LENGTH_NOTE = '2 أو 4 أو 6'
...
<title>أحكام المد العارض للسكون في القرآن الكريم</title>
<style>
:root {
  --navy:  #00BCD4;
  --navy2: #0A2040;
...
<header>
  <div class="header-title">أحكام المد العارض للسكون في القرآن الكريم</div>
  <div class="header-sub">إطالة الصوت بحرف المد عند الوقف على آخر الكلمة — 2 أو 4 أو 6 حركات</div>
  <div class="authors">
...
<div class="controls">
  ...
  <div class="ctrl-row" style="justify-content:center;">
    <button class="btn-toggle" onclick="showHelp()" style="background:#FCE4EC;color:#880E4F;border:1.5px solid #F48FB1;padding:8px 24px;">📚 تعريفات المد العارض للسكون</button>
...
<div id="allCasesOverlay" onclick="closeAllCases(event)">
  <div id="allCasesModal">
    <div id="allCasesTitle">📋 كل حالات المد العارض للسكون في هذه الآية</div>
...
<div class="legend">
  <div class="legend-item leg-lkm" id="leg-مد عارض للسكون" onclick="filterByType('مد عارض للسكون',this)">● مد عارض للسكون</div>
</div>

<!-- نافذة التعريفات -->
<div class="help-overlay" id="helpOverlay" onclick="hideHelp(event)">
  <div class="help-modal">
    <div class="help-title">📚 تعريف المد العارض للسكون</div>
    <div style="font-size:13px;color:var(--muted);text-align:center;margin-bottom:12px;padding:8px;background:#FCE4EC;border-radius:8px;">
      المد العارض للسكون: أن يقع بعد حرف المد سكونٌ عارض بسبب الوقف على آخر الكلمة، وليس سكوناً أصلياً ثابتاً في الوصل.
    </div>

    <div class="hcontent show" id="tab-lkm">
      <div class="help-def">
        <b style="color:var(--lkm)">المد العارض للسكون</b><br>
        يقع في آخر كلمة من الآية (أو أي موضع يُوقف عليه)، حين يأتي بعد حرف المد (ألف أو واو أو ياء مدية)
        حرفٌ متحرك في الوصل، فيسكَّن عرضاً بسبب الوقف عليه. يزول هذا السكون إذا وُصلت القراءة ولم يُوقف على الكلمة.
      </div>
      <div class="help-letters" style="color:var(--lkm)">مقدار المد: 2 أو 4 أو 6 حركات — والقارئ مخيّر بينها</div>
      <div class="help-example">
        <div class="help-ex-label">مثال:</div>
        ﴿الرَّحِيمِ﴾ عند الوقف عليها في نهاية آية — الياء حرف مد وبعدها سكون عارض
      </div>
    </div>
...
<div class="table-wrap">
  <div class="table-header">قائمة حالات المد العارض للسكون المستخرجة</div>
  <table>
    <thead>
      <tr>
        <th>ت</th>
...
function showAllCases() {
  if (!currentData || !currentData.cases || !currentData.cases.length) {
    alert('لا يوجد مد عارض للسكون موثّق في هذه الآية.');
    return;
  }
...
async function changeAyah(delta) {
  ...
  try {
    const r = await fetch(`/api/arth/nextayah?suraid=${currentData.suraid}&verseid=${currentData.verseid}&delta=${delta}&type=${encodeURIComponent(type)}`);
    const d = await r.json();
    showAyahNavLoading(false);
    if (d.found) {
      const p = {suraid: d.suraid, verseid: d.verseid};
      if (type !== 'الكل') p.type = type;
      const r2 = await fetch('/api/arth/ayah?' + new URLSearchParams(p));
...
async function loadSuras(type) {
  ...
  const r = await fetch(`/api/arth/suras?type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadVerses() {
  ...
  if (!suraid) return;
  const type   = activeFilter || 'الكل';
  const r = await fetch(`/api/arth/verses?suraid=${suraid}&type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadRandom() {
  ...
  const p = {random:1};
  if (type !== 'الكل') p.type = type;
  const r = await fetch('/api/arth/ayah?' + new URLSearchParams(p));
  const d = await r.json();
  if (d.error) return;
  currentData = d;
  renderAyah(d);
...
function renderAyah(d) {
  ...
  // العدادات
  const counts = {'مد عارض للسكون':0};
  d.cases.forEach(c => { if (counts[c.itype] !== undefined) counts[c.itype]++; });
  const styleMap = {
    'مد عارض للسكون': 'background:#FFF3E0;color:#FF9100;border:1px solid #FF9100',
  };
...
function playClipReciter(reciter) {
  ...
  clipPlayer.src = '/api/arth/clip?' + p.toString();
  clipPlayer.play().then(() => { status.textContent = ''; })
    .catch(async () => {
      try {
        const r = await fetch('/api/arth/clip?' + p.toString());
        const d = await r.json();
...
const WORD_QUESTIONS = {
  1: 'ما حكم هذه الكلمة من أحكام المد العارض للسكون؟',
...
async function askWordQuestion(num) {
  ...
  try {
    const r = await fetch('/api/arth/explain', {
...
def api_nextayah():
    ...
    found   = None
    try:
        cur.execute('''SELECT DISTINCT suraid, verseid FROM lastword
...
@bp.route('/api/arth/verses')
def api_verses():
    suraid = request.args.get('suraid')
    if not suraid: return jsonify([])
    conn = get_db(); cur = conn.cursor()
    try:
        cur.execute('''SELECT DISTINCT verseid FROM lastword
                       WHERE suraid=? ORDER BY CAST(verseid AS INTEGER)''', (suraid,))
        rows = cur.fetchall()
    except: rows = []
    conn.close(); return jsonify([r[0] for r in rows])


@bp.route('/api/arth/ayah')
def api_ayah():
    suraid  = request.args.get('suraid')
    ...
    if rand == '1':
        try:
            cur.execute('SELECT DISTINCT suraid,verseid FROM lastword ORDER BY RANDOM() LIMIT 1')
            r = cur.fetchone()
    ...
    # جلب نص الآية أولاً (نحتاجه لحساب موضع آخر كلمة)
    try:
    ...
    # جلب حالة المد العارض للسكون (آخر كلمة في الآية عند الوقف عليها، إن وُجدت)
    try:
        cur.execute('SELECT klmahr FROM lastword WHERE suraid=? AND verseid=? LIMIT 1',
                    (suraid, verseid))
        r = cur.fetchone()
        klmahr = (r[0] or '').strip() if r else ''
    except: klmahr = ''
    ...
    if klmahr:
        ...
        cases.append({
            'word'    : klmahr,
            'itype'   : 'مد عارض للسكون',
            'length'  : ARTH_LENGTH_NOTE,
            'position': len(aya_words) if aya_words else 1,  # آخر كلمة (1-based)
            'color'   : get_color('مد عارض للسكون'),
        })
...
@bp.route('/api/arth/explain', methods=['POST'])
def api_explain():
    ...
    try:
        system = ("You are an expert in Quran Tajweed specializing in Madd rules. "
                  ...
                  "If the database says it is Madd Arid Lissukoon (عارض للسكون), then it IS that ruling - do not deny this. "
                  ...
                  "FACT: Madd Arid Lissukoon may be read as 2, 4, or 6 harakaat - it is the reciter's choice, "
                  "and the ruling only applies when pausing (waqf) on the word; in continuous recitation (wasl) it disappears.")
