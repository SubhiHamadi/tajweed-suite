...
function bareChar(s) {
  return (s || '')
    // تجريد التشكيل الحقيقي وعلامات الوقف ومحارف التحكم غير المرئية،
    // مع استثناء الألف الخنجرية \u0670 من هذا التجريد: هي ليست تشكيلاً
    // زائداً بل تمثّل حرف الألف الطويلة فعلياً في الرسم العثماني (كما
    // في "تِّجَٰرَتُهُمْ" التي تُكتب "تِّجَارَتُهُمْ" بالإملاء القياسي)،
    // فحذفها بالكامل بدل تحويلها لألف قياسية كان يُسقط حرفاً حقيقياً من
    // هيكل المقارنة ويكسر المطابقة صامتاً في أي كلمة تحتويها.
    .replace(/[\u064B-\u065F\u0610-\u061A\u06D6-\u06ED\u08D3-\u08FF\u0640\u200B-\u200F\u202A-\u202E\u2066-\u2069\uFEFF]/g, '')
    .replace(/[\u0622\u0623\u0625\u0670\u0671]/g, '\u0627');
}
