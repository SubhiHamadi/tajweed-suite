...
ITYPE_COLORS = {
    'التقاء ياءين (الأولى مشددة)'        : '#00E5FF',
    'منع إدغام الواو المدية بالمتحركة'   : '#FFD700',
    'منع إدغام الياء المدية بالمتحركة'   : '#FF9800',
    'الحروف المدية الصغيرة (رسم المصحف)' : '#00C853',
}
...
<title>أحكام مد التمكين في القرآن الكريم</title>
<style>
:root {
  --navy:  #00BCD4;
  --navy2: #0A2040;
...
<header>
  <div class="header-title">أحكام مد التمكين في القرآن الكريم</div>
  <div class="header-sub">التقاء حرفَي مد متجانسين، أو منع إدغام حرف المد بالحرف المتحرك بعده</div>
  <div class="authors">
...
<div class="controls">
  ...
  <div class="ctrl-row" style="justify-content:center;">
    <button class="btn-toggle" onclick="showHelp()" style="background:#FCE4EC;color:#880E4F;border:1.5px solid #F48FB1;padding:8px 24px;">📚 تعريفات مد التمكين</button>
...
<div id="allCasesOverlay" onclick="closeAllCases(event)">
  <div id="allCasesModal">
    <div id="allCasesTitle">📋 كل حالات مد التمكين في هذه الآية</div>
...
<div class="legend">
  <div class="legend-item leg-lkm" id="leg-التقاء ياءين (الأولى مشددة)" onclick="filterByType('التقاء ياءين (الأولى مشددة)',this)">● التقاء ياءين</div>
  <div class="legend-item leg-lkf" id="leg-منع إدغام الواو المدية بالمتحركة" onclick="filterByType('منع إدغام الواو المدية بالمتحركة',this)">● منع إدغام الواو</div>
  <div class="legend-item leg-lhm" id="leg-منع إدغام الياء المدية بالمتحركة" onclick="filterByType('منع إدغام الياء المدية بالمتحركة',this)">● منع إدغام الياء</div>
  <div class="legend-item leg-lhf" id="leg-الحروف المدية الصغيرة (رسم المصحف)" onclick="filterByType('الحروف المدية الصغيرة (رسم المصحف)',this)">● حروف مدية صغيرة</div>
</div>

<!-- نافذة التعريفات -->
<div class="help-overlay" id="helpOverlay" onclick="hideHelp(event)">
  <div class="help-modal">
    <div class="help-title">📚 تعريفات مد التمكين</div>
    <div style="font-size:13px;color:var(--muted);text-align:center;margin-bottom:12px;padding:8px;background:#FCE4EC;border-radius:8px;">
      مد التمكين: يقع عند التقاء حرفَي مد متجانسين (كالياء الساكنة بعد ياء مشددة مكسورة)، أو لمنع إدغام حرف مد بحركة بعده
    </div>
    <div class="help-tabs">
      <button class="help-tab help-tab-lkm" onclick="showTab('lkm')">التقاء ياءين</button>
      <button class="help-tab help-tab-lkf" onclick="showTab('lkf')">منع إدغام الواو</button>
      <button class="help-tab help-tab-lhm" onclick="showTab('lhm')">منع إدغام الياء</button>
      <button class="help-tab help-tab-lhf" onclick="showTab('lhf')">حروف مدية صغيرة</button>
    </div>

    <div class="hcontent show" id="tab-lkm">
      <div class="help-def">
        <b style="color:var(--lkm)">مد التمكين — التقاء ياءين (الأولى مشددة)</b><br>
        أن تأتي ياء ساكنة مدية بعد ياء مشددة مكسورة، فتتمكن الياء المدية في النطق لضمان ظهورها ولا تختفي في التشديد.
      </div>
      <div class="help-letters" style="color:var(--lkm)">مقدار المد: حركتان (كالمد الطبيعي)</div>
      <div class="help-example">
        <div class="help-ex-label">مثال:</div>
        ﴿النَّبِيِّـۧنَ﴾ &nbsp;·&nbsp; ﴿حُيِّيتُم﴾
      </div>
    </div>

    <div class="hcontent" id="tab-lkf">
      <div class="help-def">
        <b style="color:var(--lkf)">مد التمكين — منع إدغام الواو المدية بالمتحركة</b><br>
        أن تأتي واو مدية ساكنة بعد واو متحركة، فتُمنع من الإدغام فيها وتظهر مداً واضحاً.
      </div>
      <div class="help-letters" style="color:var(--lkf)">مقدار المد: حركتان (كالمد الطبيعي)</div>
      <div class="help-example">
        <div class="help-ex-label">مثال:</div>
        ﴿ءَامَنُوا وَعَمِلُوا﴾ عند وصل الواوين
      </div>
    </div>

    <div class="hcontent" id="tab-lhm">
      <div class="help-def">
        <b style="color:var(--lhm)">مد التمكين — منع إدغام الياء المدية بالمتحركة</b><br>
        أن تأتي ياء مدية ساكنة بعد ياء متحركة، فتُمنع من الإدغام فيها وتظهر مداً واضحاً.
      </div>
      <div class="help-letters" style="color:var(--lhm)">مقدار المد: حركتان (كالمد الطبيعي)</div>
      <div class="help-example">
        <div class="help-ex-label">مثال:</div>
        ﴿فِي يَوْمٍ﴾ عند وصل الياءين
      </div>
    </div>

    <div class="hcontent" id="tab-lhf">
      <div class="help-def">
        <b style="color:var(--lhf)">مد التمكين — الحروف المدية الصغيرة (رسم المصحف)</b><br>
        حروف مد صغيرة مرسومة فوق السطر في المصحف العثماني للدلالة على مد التمكين في مواضع مخصوصة.
      </div>
      <div class="help-letters" style="color:var(--lhf)">مقدار المد: حركتان (كالمد الطبيعي)</div>
      <div class="help-example">
        <div class="help-ex-label">مثال:</div>
        الحروف الصغيرة الملحقة برسم بعض الكلمات في المصحف
      </div>
    </div>
...
<div class="table-wrap">
  <div class="table-header">قائمة حالات مد التمكين المستخرجة</div>
  <table>
    <thead>
      <tr>
        <th>ت</th>
...
function showAllCases() {
  if (!currentData || !currentData.cases || !currentData.cases.length) {
    alert('لا يوجد مد تمكين موثّق في هذه الآية.');
    return;
  }
...
async function changeAyah(delta) {
  ...
  try {
    const r = await fetch(`/api/tmkeen/nextayah?suraid=${currentData.suraid}&verseid=${currentData.verseid}&delta=${delta}&type=${encodeURIComponent(type)}`);
    const d = await r.json();
    showAyahNavLoading(false);
    if (d.found) {
      const p = {suraid: d.suraid, verseid: d.verseid};
      if (type !== 'الكل') p.type = type;
      const r2 = await fetch('/api/tmkeen/ayah?' + new URLSearchParams(p));
...
async function loadSuras(type) {
  ...
  const r = await fetch(`/api/tmkeen/suras?type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadVerses() {
  ...
  if (!suraid) return;
  const type   = activeFilter || 'الكل';
  const r = await fetch(`/api/tmkeen/verses?suraid=${suraid}&type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadRandom() {
  ...
  const p = {random:1};
  if (type !== 'الكل') p.type = type;
  const r = await fetch('/api/tmkeen/ayah?' + new URLSearchParams(p));
  const d = await r.json();
  if (d.error) return;
  currentData = d;
  renderAyah(d);
...
function renderAyah(d) {
  ...
  // العدادات
  const counts = {'التقاء ياءين (الأولى مشددة)':0,'منع إدغام الواو المدية بالمتحركة':0,'منع إدغام الياء المدية بالمتحركة':0,'الحروف المدية الصغيرة (رسم المصحف)':0};
  d.cases.forEach(c => { if (counts[c.itype] !== undefined) counts[c.itype]++; });
  const styleMap = {
    'التقاء ياءين (الأولى مشددة)': 'background:#E0F7FA;color:#00E5FF;border:1px solid #00E5FF',
    'منع إدغام الواو المدية بالمتحركة': 'background:#FFF8E1;color:#FFD700;border:1px solid #FFD700',
    'منع إدغام الياء المدية بالمتحركة': 'background:#FFF3E0;color:#FF9800;border:1px solid #FF9800',
    'الحروف المدية الصغيرة (رسم المصحف)': 'background:#E8F5E9;color:#00C853;border:1px solid #00C853',
  };
  document.getElementById('infoCounts').innerHTML = Object.entries(counts)
    .filter(([,n])=>n>0)
    .map(([k,n])=>`<span class="cnt-badge" style="${styleMap[k]}">${k}: ${n}</span>`)
    .join('');
...
function playClipReciter(reciter) {
  ...
  clipPlayer.src = '/api/tmkeen/clip?' + p.toString();
  clipPlayer.play().then(() => { status.textContent = ''; })
    .catch(async () => {
      try {
        const r = await fetch('/api/tmkeen/clip?' + p.toString());
        const d = await r.json();
...
const WORD_QUESTIONS = {
  1: 'ما حكم هذه الكلمة من أحكام مد التمكين؟',
...
async function askWordQuestion(num) {
  ...
  try {
    const r = await fetch('/api/tmkeen/explain', {
...
def api_nextayah():
    ...
    try:
        if itype and itype != 'الكل':
            cur.execute('''SELECT DISTINCT suraid, verseid FROM tamkeendirect
                           WHERE itype=?
...
@bp.route('/api/tmkeen/suras')
def api_suras():
    itype = request.args.get('type','الكل')
    conn = get_db(); cur = conn.cursor()
    try:
        if itype and itype != 'الكل':
            cur.execute('SELECT DISTINCT suraid, suraname FROM tamkeendirect WHERE itype=? ORDER BY CAST(suraid AS INTEGER)', (itype,))
        else:
            cur.execute('SELECT DISTINCT suraid, suraname FROM tamkeendirect ORDER BY CAST(suraid AS INTEGER)')
        rows = [{'suraid':r[0],'suraname':r[1]} for r in cur.fetchall()]
    except: rows = []
    conn.close(); return jsonify(rows)


@bp.route('/api/tmkeen/verses')
def api_verses():
    ...
    try:
        if itype and itype != 'الكل':
            cur.execute('SELECT DISTINCT verseid FROM tamkeendirect WHERE suraid=? AND itype=? ORDER BY CAST(verseid AS INTEGER)', (suraid, itype))
        else:
            cur.execute('SELECT DISTINCT verseid FROM tamkeendirect WHERE suraid=? ORDER BY CAST(verseid AS INTEGER)', (suraid,))
        rows = cur.fetchall()
    except: rows = []
    conn.close(); return jsonify([r[0] for r in rows])


@bp.route('/api/tmkeen/ayah')
def api_ayah():
    suraid  = request.args.get('suraid')
    ...
    if rand == '1':
        try:
            if itype and itype != 'الكل':
                cur.execute('SELECT DISTINCT suraid,verseid FROM tamkeendirect WHERE itype=? ORDER BY RANDOM() LIMIT 1', (itype,))
            else:
                cur.execute('SELECT DISTINCT suraid,verseid FROM tamkeendirect ORDER BY RANDOM() LIMIT 1')
            r = cur.fetchone()
    ...
    # جلب الأحكام (جدول tamkeendirect مستقل بذاته: يضم أيضاً نص الآية واسم السورة)
    try:
        if itype and itype != 'الكل':
            cur.execute('''SELECT klmat, itype, NULL, kseq
                           FROM tamkeendirect
                           WHERE suraid=? AND verseid=? AND itype=?
    ...
    # جلب نص الآية (من نفس جدول tamkeendirect، لا حاجة لربط مع mushafnew)
    try:
        cur.execute('''SELECT suraname, aya, pagenum
                       FROM tamkeendirect WHERE suraid=? AND verseid=? LIMIT 1''',
                    (suraid, verseid))
...
@bp.route('/api/tmkeen/explain', methods=['POST'])
def api_explain():
    ...
    try:
        system = ("You are an expert in Quran Tajweed specializing in Madd rules. "
                  ...
                  "If the database says it is Madd Al-Tamkeen, then it IS that ruling - do not deny this. "
                  ...
                  "FACT: Madd Al-Tamkeen occurs at the meeting of two homogeneous madd letters "
                  "(e.g. a saakin yaa after a shaddah-kasrah yaa) or to prevent a madd letter from "
                  "being merged (idgham) into a following moving letter; its length is 2 harakaat, "
                  "like a normal madd.")
...
def api_clip():
    ...
    conn = get_db(); cur = conn.cursor()
    cur.execute('SELECT aya FROM tamkeendirect WHERE suraid=? AND verseid=? LIMIT 1', (suraid, verseid))
