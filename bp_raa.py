"""
raa_web_app.py — أحكام الراء (التفخيم والترقيق) في القرآن الكريم
نسخة الويب، مستوى الصفحة. نسخة Flask من raa_page_app.py، بنفس هوية
ikhfa_web_app.py وizhar_web_app.py البصرية.

ملاحظة: النسخة الأصلية تعتمد على sabab_mapping.py (غير مرفوع) لتجميع
أسباب الترقيق/التفخيم في فئات مرقّمة (مثل: "١. راء مفتوحة أو مضمومة").
هذه النسخة تستخدم بدلاً من ذلك قائمة تصفية مسطّحة بكل نص سبب موجود
فعلياً في قاعدة البيانات (نفس أسلوب فلتر السبب في izhar_web_app.py
وmim_web_app.py) — إن أردت التجميع بالفئات لاحقاً، أرسل sabab_mapping.py.
يعمل على بورت 5047.
"""
...
# ══════════════════════════════════════════════════════════════
#  ألوان/أيقونات/تعريفات أحكام الراء — منقولة حرفياً من raa_page_app.py
# ══════════════════════════════════════════════════════════════
C_GOLD2 = '#F0C755'
HOKM_COLOR = {
    'تفخيم'        : '#FF4081',
...
def get_color(hokm):
    return HOKM_COLOR.get(hokm, C_GOLD2)
...
def get_icon(hokm):
    return HOKM_ICON.get(hokm, '🔹')

HOKM_DEFINITIONS = {
    'تفخيم'       : 'سُمنة تحدث في جسم الحرف عند النطق به، فيمتلئ الفم بصداه وصوته.',
    'ترقيق'       : 'نحافة تحدث في جسم الحرف عند النطق به، فلا يمتلئ الفم بصداه.',
    'جواز_الوجهين': 'يجوز للقارئ تفخيم الراء أو ترقيقها عند هذه الكلمة، وكلا الوجهين صحيح ومقبول عند علماء التجويد.',
}

# ══════════════════════════════════════════════════════════════
#  تصنيف الأسباب إلى فئات تعليمية مرقّمة — نفس منطق sabab_mapping.py
#  الأصلي حرفياً. يُستورَد من ملف مستقل إن وُجد بجانب هذا الملف
#  (يسمح بتحديثه دون لمس هذا الملف)، وإلا تُستخدم هذه النسخة المضمَّنة.
# ══════════════════════════════════════════════════════════════
try:
    from sabab_mapping import get_category, get_categories_list, get_sabab_list_for_category
except Exception:
    TAFKHIM_CATEGORIES = {
        'مفتوحة'                                              : (1, 'إذا كانت مفتوحة'),
        'مفتوحة (مشددة)'                                      : (1, 'إذا كانت مفتوحة'),
        'مضمومة'                                              : (2, 'إذا كانت مضمومة'),
        'مضمومة (مشددة)'                                      : (2, 'إذا كانت مضمومة'),
        'وقف: قبلها فتح مباشرة (نادر)'                        : (3, 'إذا سكنت وقبلها غير مكسور في آية'),
        'وقف: قبلها ضم مباشرة (نادر)'                         : (3, 'إذا سكنت وقبلها غير مكسور في آية'),
        'ساكنة وقبلها فتح'                                     : (4, 'إذا كانت ساكنة وقبلها مفتوح'),
        'ساكنة وقبلها ضم'                                      : (5, 'إذا كانت ساكنة وقبلها ضم'),
        'ساكنة مسبوقة بهمزة وصل (كسر عارض للابتداء)'          : (6, 'إذا كانت ساكنة مسبوقة بهمزة وصل'),
        'ساكنة، قبلها كسر، بعدها استعلاء مفتوح'                : (7, 'إذا كانت ساكنة وقبلها كسر أصلي وبعدها حرف استعلاء وغير مكسور في كلمة واحدة'),
        'وقف: قبلها ساكن وقبل الساكن فتح'                      : (8, 'إذا كانت ساكنة مسبوقة بساكن مسبوق بفتح'),
        'وقف: قبلها ساكن وقبل الساكن ضم'                       : (9, 'إذا كانت ساكنة مسبوقة بساكن مسبوق بضم'),
        'وقف: قبلها حرف مد (ا) يتبع فتح'                       : (4, 'إذا كانت ساكنة وقبلها مفتوح'),
        'وقف: قبلها حرف مد (و) يتبع ضم'                        : (5, 'إذا كانت ساكنة وقبلها ضم'),
        'راء محذوفة للتخفيف (ياء محذوفة رسماً) - فَأَسْرِ'    : (0, 'حالة خاصة: راء محذوفة للتخفيف'),
        'راء محذوفة للتخفيف (ياء محذوفة رسماً) - وَنُذُرِ'    : (0, 'حالة خاصة: راء محذوفة للتخفيف'),
        'راء محذوفة للتخفيف (ياء محذوفة رسماً) - أَسْرِ'      : (0, 'حالة خاصة: راء محذوفة للتخفيف'),
        'راء محذوفة للتخفيف (ياء محذوفة رسماً) - يَسْرِ'      : (0, 'حالة خاصة: راء محذوفة للتخفيف'),
        'فاتحة سورة (را المقطّعة) - مد بالفتح'                : (0, 'حالة خاصة: فاتحة سورة (را المقطّعة)'),
    }
    TARQIQ_CATEGORIES = {
        'مكسورة'                                               : (1, 'إذا كانت مكسورة'),
        'مكسورة (مشددة)'                                       : (1, 'إذا كانت مكسورة'),
        'ساكنة وقبلها كسر أصلي (لا استعلاء بعدها)'             : (4, 'إذا كانت ساكنة وقبلها كسر أصلي وليس بعدها حرف استعلاء'),
        'وقف: قبلها كسر مباشرة (نادر)'                         : (2, 'إذا سكنت وقبلها مكسور في آية'),
        'وقف: قبلها ياء ساكنة'                                 : (3, 'إذا سكنت وقبلها ياء ساكنة'),
        'وقف: قبلها ياء مدّية'                                 : (3, 'إذا سكنت وقبلها ياء ساكنة'),
        'وقف: قبلها ساكن وقبل الساكن كسر'                      : (5, 'إذا سكنت وقفاً وكان قبلها ساكن وقبل الساكن كسر'),
    }
    JAWAZ_CATEGORIES = {
        'ساكنة، قبلها كسر، بعدها استعلاء مكسور'                       : 'الراء ساكنة وقبلها كسر وبعدها حرف استعلاء مكسور (فِرْق)',
        'مِصْر - يجوز تفخيمها أو ترقيقها وقفاً (الأولى: تفخيم)'        : 'مِصْر: ساكنة للوقف وقبلها استعلاء ساكن قبله مكسور (الأولى: تفخيم)',
        'القِطْر - يجوز تفخيمها أو ترقيقها وقفاً (الأولى: ترقيق)'      : 'القِطْر: ساكنة للوقف وقبلها استعلاء ساكن قبله مكسور (الأولى: ترقيق)',
    }

    def get_category(hokm, sabab):
        if hokm == 'تفخيم':
            return TAFKHIM_CATEGORIES.get(sabab, (0, sabab))
        elif hokm == 'ترقيق':
            return TARQIQ_CATEGORIES.get(sabab, (0, sabab))
        elif hokm == 'جواز_الوجهين':
            return (0, JAWAZ_CATEGORIES.get(sabab, sabab))
        return (0, sabab)

    def get_categories_list(hokm):
        if hokm == 'تفخيم':
            seen = {}
            for sabab, (num, name) in TAFKHIM_CATEGORIES.items():
                if num > 0 and num not in seen:
                    seen[num] = name
            return sorted(seen.items())
        elif hokm == 'ترقيق':
            seen = {}
            for sabab, (num, name) in TARQIQ_CATEGORIES.items():
                if num > 0 and num not in seen:
                    seen[num] = name
            return sorted(seen.items())
        elif hokm == 'جواز_الوجهين':
            names = list(dict.fromkeys(JAWAZ_CATEGORIES.values()))
            return [(i + 1, name) for i, name in enumerate(names)]
        return []

    def get_sabab_list_for_category(hokm, cat_num):
        if hokm == 'تفخيم':
            source = TAFKHIM_CATEGORIES
        elif hokm == 'ترقيق':
            source = TARQIQ_CATEGORIES
        elif hokm == 'جواز_الوجهين':
            names = list(dict.fromkeys(JAWAZ_CATEGORIES.values()))
            if 1 <= cat_num <= len(names):
                target_name = names[cat_num - 1]
                return [s for s, n in JAWAZ_CATEGORIES.items() if n == target_name]
            return []
        else:
            return []
        return [sabab for sabab, (num, name) in source.items() if num == cat_num]
...
#  raa_page_app.py، مع تبسيط فلتر السبب إلى قيمة واحدة مباشرة
#  (بدل قائمة فئة متعددة القيم من sabab_mapping.py غير المتوفر)
# ══════════════════════════════════════════════════════════════
def _load_page(page_num, hokm_filter=None, cat_num=None):
    sabab_list = get_sabab_list_for_category(hokm_filter, cat_num) if (hokm_filter and cat_num) else None
    conn = get_db(); cur = conn.cursor()
    result = []
    try:
        params, where = [page_num], ['m.PAGENUM=?']
        if hokm_filter and hokm_filter != 'الكل':
            where.append('r.hokm=?'); params.append(hokm_filter)
        if sabab_list:
            placeholders = ','.join('?' * len(sabab_list))
            where.append(f'r.sabab IN ({placeholders})'); params.extend(sabab_list)
        ...
        for suraid, verseid, suraname, aya_text in ayas:
            ...
            if hokm_filter and hokm_filter != 'الكل':
                w2.append('hokm=?'); p2.append(hokm_filter)
            if sabab_list:
                placeholders = ','.join('?' * len(sabab_list))
                w2.append(f'sabab IN ({placeholders})'); p2.extend(sabab_list)
            cur.execute(f"""
                SELECT id, klma, position, hokm, sabab, prev_char, next_char
                FROM raa_rules WHERE {' AND '.join(w2)}
                ORDER BY position
            """, p2)
            rows = cur.fetchall()
            cases = []
            for cid, klma, position, hokm, sabab, prev_char, next_char in rows:
                cat_n, cat_name = get_category(hokm, sabab)
                cases.append({
                    'id': cid, 'klma': (klma or '').strip(), 'position': position,
                    'hokm': hokm, 'sabab': sabab, 'prev_char': prev_char, 'next_char': next_char,
                    'color': get_color(hokm), 'icon': get_icon(hokm),
                    'cat_display': f'{cat_n}. {cat_name}' if cat_n > 0 else (sabab or ''),
                })
            result.append({
...
def _find_nearest_page(current_page, direction, hokm_filter=None, cat_num=None):
    sabab_list = get_sabab_list_for_category(hokm_filter, cat_num) if (hokm_filter and cat_num) else None
    conn = get_db(); cur = conn.cursor()
    try:
        params, where = [], []
        ...
        if direction == 1:
            cur.execute(f"""
                SELECT DISTINCT m.PAGENUM FROM raa_rules r
                JOIN mushafnew m ON m.SURAID=r.suraid AND m.VERSEID=r.verseid
...
def _random_page(hokm_filter=None, cat_num=None):
    sabab_list = get_sabab_list_for_category(hokm_filter, cat_num) if (hokm_filter and cat_num) else None
    conn = get_db(); cur = conn.cursor()
    try:
        params, where = [], []
...
<title>أحكام الراء (التفخيم والترقيق) في القرآن الكريم</title>
<style>
:root {
...
<header>
  <div class="header-title">أحكام الراء في القرآن الكريم</div>
  <div class="header-sub">تفخيم، ترقيق، جواز الوجهين</div>
  <div class="authors">
...
<div class="controls">
  ...
  <div class="ctrl-row">
    ...
    <select id="selHokm" onchange="onHokmChange()"></select>
    <span class="ctrl-label">الفئة:</span>
    <select id="selCategory" onchange="loadPage(currentPage)"></select>
  </div>
  <div class="ctrl-row">
...
<div class="table-wrap">
  <div class="table-header" style="display:flex;justify-content:space-between;align-items:center;">
    <span>📋 كل حالات الراء في هذه الصفحة</span>
    <button class="btn-showall" onclick="showAllCases()">⛶ عرض الكل</button>
  </div>
  <table>
    <thead><tr>
      <th>السورة</th><th>الآية</th><th>الكلمة</th><th>الحكم</th><th>السبب</th>
    </tr></thead>
...
<div id="allCasesOverlay" onclick="closeAllCases(event)">
  <div class="all-cases-modal">
    <div class="table-header" style="margin:-20px -20px 12px;border-radius:16px 16px 0 0;">📋 كل حالات الراء في هذه الصفحة</div>
    <table>
      <thead><tr>
        <th>السورة</th><th>الآية</th><th>الكلمة</th><th>الحكم</th><th>السبب</th>
      </tr></thead>
...
const HOKM_COLORS = {'تفخيم':'#FF4081','ترقيق':'#00E5FF','جواز_الوجهين':'#FFD700'};
const HOKM_DEFS = {
  'تفخيم': 'سُمنة تحدث في جسم الحرف عند النطق به، فيمتلئ الفم بصداه وصوته.',
  'ترقيق': 'نحافة تحدث في جسم الحرف عند النطق به، فلا يمتلئ الفم بصداه.',
  'جواز_الوجهين': 'يجوز للقارئ تفخيم الراء أو ترقيقها عند هذه الكلمة، وكلا الوجهين صحيح ومقبول عند علماء التجويد.',
...
function buildLegend() {
  document.getElementById('legend').innerHTML = Object.entries(HOKM_COLORS).map(([name,color]) =>
    `<span class="legend-item" style="background:${color}22;color:${color};border:2px solid ${color};"
      onclick="filterByHokm('${name}')">● ${name.replace('_',' ')}</span>`).join('');
  const selHokm = document.getElementById('selHokm');
  selHokm.innerHTML = '<option value="الكل">الكل</option>' +
    Object.keys(HOKM_COLORS).map(n => `<option value="${n}">${n.replace('_',' ')}</option>`).join('');
}

async function loadCategories() {
  const hokm = document.getElementById('selHokm').value || 'الكل';
  const sel = document.getElementById('selCategory');
  if (hokm === 'الكل') {
    sel.innerHTML = '<option value="0">الكل</option>';
    sel.disabled = true;
    return;
  }
  sel.disabled = false;
  const r = await fetch(`/api/raa/categories?hokm=${encodeURIComponent(hokm)}`);
  const d = await r.json();
  sel.innerHTML = '<option value="0">الكل</option>' +
    d.map(c => `<option value="${c.num}">${c.num}. ${c.name}</option>`).join('');
}

function filterByHokm(name) { document.getElementById('selHokm').value = name; onHokmChange(); }
async function onHokmChange() { await loadCategories(); loadPage(currentPage); }
...
function renderPage(data) {
  ...
  if (!pageVerses.length) {
    container.innerHTML = '';
    showNotify('لا يوجد حكم راء موثّق في هذه الصفحة (أو حسب الفلتر الحالي) — استخدم ◄ أو ► للانتقال لأقرب صفحة.');
  ...
  container.innerHTML = pageVerses.map(v => {
    ...
    v.cases.forEach(c => {
      if (!c.klma) return;
      const [lo, hi] = findWordRange(words, c.klma);
      if (lo === -1) return;
  ...
  pageVerses.forEach(v => {
    ...
    v.cases.forEach((c, ci) => {
      rows.push(`<tr class="case-row" data-key="${key}" data-caseidx="${ci}">
        ...
        <td class="word-cell" style="color:${c.color};">${c.klma}</td>
        <td style="color:${c.color};">${c.icon} ${c.hokm.replace('_',' ')}</td>
        <td style="font-size:11px;color:var(--muted);">${c.cat_display || ''}</td>
      </tr>`);
    });
  });
...
async function loadPage(page) {
  ...
  const hokm = document.getElementById('selHokm').value || 'الكل';
  const cat  = document.getElementById('selCategory').value || '0';
  const r = await fetch(`/api/raa/page?page=${page}&hokm=${encodeURIComponent(hokm)}&cat=${cat}`);
  const d = await r.json();
  renderPage(d);
}
...
async function jumpPage(direction) {
  const hokm = document.getElementById('selHokm').value || 'الكل';
  const cat  = document.getElementById('selCategory').value || '0';
  const r = await fetch(`/api/raa/nearest?page=${currentPage}&dir=${direction}&hokm=${encodeURIComponent(hokm)}&cat=${cat}`);
  const d = await r.json();
...
async function loadRandomPage() {
  const hokm = document.getElementById('selHokm').value || 'الكل';
  const cat  = document.getElementById('selCategory').value || '0';
  const r = await fetch(`/api/raa/random?hokm=${encodeURIComponent(hokm)}&cat=${cat}`);
  const d = await r.json();
  loadPage(d.page || 1);
}
...
function openSelectionExplain(key, lo, hi) {
  ...
  document.getElementById('wordContext').textContent =
    ...
    (rulings.length ? ' — ' + rulings.map(r => `${r.hokm.replace('_',' ')} (${r.cat_display || r.sabab || ''})`).join('، ') : '');
  document.getElementById('wordResult').textContent =
    rulings.length ? (HOKM_DEFS[rulings[0].hokm] || '') : 'كلمة عادية بلا حكم راء موثّق هنا.';
...
function playClipReciter(reciter) {
  ...
  clipPlayer.src = '/api/raa/clip?' + p.toString();
  clipPlayer.play().then(() => { status.textContent = ''; })
    .catch(async () => {
      try {
        const r = await fetch('/api/raa/clip?' + p.toString());
...
@bp.route('/api/raa/page')
def api_page():
    page = request.args.get('page', 1, type=int)
    hokm_filter = request.args.get('hokm', 'الكل')
    cat_num     = request.args.get('cat', 0, type=int)
    verses = _load_page(page, hokm_filter, cat_num)
    return jsonify({'page': page, 'verses': verses})

@bp.route('/api/raa/nearest')
def api_nearest():
    page      = request.args.get('page', 1, type=int)
    direction = request.args.get('dir', 1, type=int)
    hokm_filter = request.args.get('hokm', 'الكل')
    cat_num     = request.args.get('cat', 0, type=int)
    return jsonify({'page': _find_nearest_page(page, direction, hokm_filter, cat_num)})

@bp.route('/api/raa/random')
def api_random():
    hokm_filter = request.args.get('hokm', 'الكل')
    cat_num     = request.args.get('cat', 0, type=int)
    return jsonify({'page': _random_page(hokm_filter, cat_num)})

@bp.route('/api/raa/categories')
def api_categories():
    """يرجع قائمة الفئات التعليمية المرقّمة لحكم مُحدَّد (تفخيم/ترقيق/
    جواز_الوجهين)، من sabab_mapping.py — فارغة إن كان الحكم 'الكل'."""
    hokm = request.args.get('hokm', 'الكل')
    if not hokm or hokm == 'الكل':
        return jsonify([])
    cats = get_categories_list(hokm)
    return jsonify([{'num': n, 'name': name} for n, name in cats])

@bp.route('/api/raa/clip')
def api_clip():
...
@bp.route('/api/raa/explain', methods=['POST'])
def api_explain():
    ...
    question = data.get('question', f'ما حكم الراء في كلمة {word} في القرآن الكريم؟')
    ...
    try:
        system = ("You are an expert in Quran Tajweed specializing in Raa Tafkheem/Tarqeeq "
                  "(emphasis/softening) rules. Answer briefly in max 3-4 lines. "
