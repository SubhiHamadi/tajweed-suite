"""
alifat_web_app.py — الألفات السبع في القرآن الكريم (نسخة الويب)
نسخة Flask من alifat_sabaa_app.py، بنفس هوية بقية السويطة البصرية.
يعتمد على جدول alifatdirect (يُبنى مسبقاً عبر build_alefat_table.py —
تأكد من تشغيله على quran.db قبل تشغيل هذا التطبيق أول مرة).
التنقل: سورة ← موضع (آية) ضمنها، مع فلتر بالنوع وزر عشوائي — نفس
نمط alifat_sabaa_app.py الأصلي تماماً (٦٧ موضعاً فقط، موزّعة على
عدد محدود من السور، فهذا أنسب من التنقل بالصفحة).
يعمل على بورت 5051.
"""
import os, re, sys, sqlite3, json
from flask import Flask, Blueprint, jsonify, request, send_from_directory, send_file

bp = Blueprint('alifat', __name__)
...
# ══════════════════════════════════════════════════════════════
#  ألوان/تعريفات أنواع الألفات السبع — منقولة حرفياً من alifat_sabaa_app.py
# ══════════════════════════════════════════════════════════════
TYPE_COLOR = {
    'أنا'    : '#F0C755',
    'لكنا'   : '#00E5FF',
...
def get_color(itype):
    return TYPE_COLOR.get(itype, '#F0C755')

TYPE_DEFINITIONS = {
    'أنا': 'ألف زائدة رسماً بعد "أنا" (وكذا وأنا/فأنا)، تُثبَت وقفاً وتُحذف وصلاً، ولا تُنطق في وسط الكلام.',
    'لكنا': 'في قوله تعالى "لَّٰكِنَّا۠ هُوَ اللَّهُ رَبِّي" (الكهف ٣٨) — ألف زائدة تُثبَت وقفاً فقط.',
    'الظنونا': 'في قوله تعالى "وَتَظُنُّونَ بِاللَّهِ ٱلظُّنُونَا۠" (الأحزاب ١٠) — ألف زائدة تُثبَت وقفاً فقط، مراعاة لفواصل الآيات.',
    'الرسولا': 'في قوله تعالى "وَأَطَعْنَا ٱلرَّسُولَا۠" (الأحزاب ٦٦) — ألف زائدة تُثبَت وقفاً فقط.',
    'السبيلا': 'في قوله تعالى "أَضَلُّونَا ٱلسَّبِيلَا۠" (الأحزاب ٦٧) — ألف زائدة تُثبَت وقفاً فقط.',
    'قواريرا': 'في قوله تعالى "قَوَارِيرَا۠" (الإنسان ١٥) — ألف زائدة تُثبَت وقفاً فقط (بخلاف قوارير الثانية في الآية ١٦ التي لا ألف فيها).',
    'سلاسلا': 'في قوله تعالى "سَلَاسِلَا۠" (الإنسان ٤) — تُقرأ بلا تنوين عند حفص، وتُثبَت الألف وقفاً اتّباعاً لرسم المصحف.',
}
...
# ══════════════════════════════════════════════════════════════
#  منطق التحميل — منقول من LoadThread في alifat_sabaa_app.py دون أي
...
def _load_ayah(suraid, verseid):
    ...
    try:
        cur.execute('''
            ...
            FROM alifatdirect WHERE suraid=? AND verseid=?
            ORDER BY kseq
        ''', (suraid, verseid))
        rows = cur.fetchall()
    except Exception:
    ...
    if not rows:
        return None
    cases = [{'suraid': r[0], 'verseid': r[1], 'klmat': r[2], 'kseq': r[3],
              'pagenum': r[4], 'itype': r[5], 'color': get_color(r[5]),
              'definition': TYPE_DEFINITIONS.get(r[5], '')} for r in rows]
    return {
        'suraid': rows[0][0], 'verseid': rows[0][1], 'suraname': rows[0][6],
        'aya': _clean_aya(rows[0][7]), 'page': rows[0][4], 'cases': cases,
    }

def _random_ayah(type_filter=None):
    conn = get_db(); cur = conn.cursor()
    try:
        params, where = [], []
        if type_filter and type_filter != 'الكل':
            where.append('itype=?'); params.append(type_filter)
        wclause = ('WHERE ' + ' AND '.join(where)) if where else ''
        cur.execute(f'SELECT DISTINCT suraid,verseid FROM alifatdirect {wclause} ORDER BY RANDOM() LIMIT 1', params)
        r = cur.fetchone()
    except Exception:
        r = None
    conn.close()
...
<title>الألفات السبع في القرآن الكريم</title>
<style>
:root {
...
select {
  ...
  font-size:13px; flex:1; min-width:80px; cursor:pointer; color-scheme: dark;
}
select option { background:#0A2040; color:var(--text); }
...
.detail-row { margin-bottom:10px; font-size:13px; line-height:185%; }
.detail-row:last-child { margin-bottom:0; }
.detail-label { font-weight:bold; display:block; margin-bottom:3px; }
...
<header>
  <div class="header-title">الألفات السبع في القرآن الكريم</div>
  <div class="header-sub">مواضع سبعة تُثبَت فيها الألف وقفاً وتُحذف وصلاً حسب رسم المصحف</div>
  <div class="authors">
...
<div class="controls">
  <div class="ctrl-row">
    ...
    <select id="selCase" onchange="onCaseChange()"></select>
  </div>
  <div class="ctrl-row">
    <span class="ctrl-label">النوع:</span>
...
let currentWords = [];
let currentRanges = [];   // [{lo,hi,case}, ...] لكل موضع ألف في هذه الآية
...
async function loadTypes() {
  const r = await fetch('/api/alifat/types');
  const d = await r.json();
  document.getElementById('selType').innerHTML =
    '<option value="الكل">الكل</option>' +
    d.map(t => `<option value="${t.itype}">${t.itype} (${t.count})</option>`).join('');
  document.getElementById('legend').innerHTML = d.map(t =>
    `<span class="legend-item" style="background:${t.color}22;color:${t.color};border:2px solid ${t.color};"
      onclick="filterByType('${t.itype}')">● ${t.itype}</span>`).join('');
}
...
async function loadSuras() {
  const r = await fetch('/api/alifat/suras');
  const d = await r.json();
  document.getElementById('selSura').innerHTML =
    d.map(s => `<option value="${s.suraid}">${s.suraid}. ${s.suraname}</option>`).join('');
}

async function loadCasesForSura(suraid) {
  const r = await fetch(`/api/alifat/cases?suraid=${suraid}`);
  const d = await r.json();
  document.getElementById('selCase').innerHTML =
    d.map(c => `<option value="${c.verseid}">آية ${c.verseid} — [${c.itype}] ${c.klmat}</option>`).join('');
  return d;
}

async function onSuraChange() {
  const suraid = document.getElementById('selSura').value;
  const cases = await loadCasesForSura(suraid);
  if (cases.length) loadAyah(suraid, cases[0].verseid);
}
function onCaseChange() {
  const suraid  = document.getElementById('selSura').value;
  const verseid = document.getElementById('selCase').value;
  if (suraid && verseid) loadAyah(suraid, verseid);
}
...
// يزيل التشكيل ويوحّد صور الألف قبل المقارنة (نفس منطق باقي السويطة
// لتفادي اختلافات الرسم العثماني بين نص quran.db وبيانات الحالة)
function bareChar(s) {
  return (s || '')
...
function renderAyah(d) {
  ...
  document.getElementById('infoAya').textContent = `سورة ${d.suraname} — الآية ${d.verseid}${pageStr}`;
  document.getElementById('infoCount').textContent = `${d.cases.length} موضع`;
  hideNotify();

  const words = (d.aya || '').split(/\s+/).filter(Boolean);
  currentWords = words;
  currentRanges = d.cases.map(c => {
    const [lo, hi] = findWordRange(words, c.klmat);
    return { lo, hi, case: c };
  }).filter(r => r.lo !== -1);

  document.getElementById('ayahBox').innerHTML = words.map((w, i) => {
    const hit = currentRanges.find(r => i >= r.lo && i <= r.hi);
    return hit
      ? `<span class="word-tok" data-idx="${i}" style="color:${hit.case.color};font-weight:bold;">${w}</span>`
      : `<span class="word-tok" data-idx="${i}" style="color:var(--text)">${w}</span>`;
  }).join(' ');
  initDragSelect();
  clearSelection();

  document.getElementById('detailCard').innerHTML = d.cases.map(c => `
    <div class="detail-row">
      <span class="detail-label" style="color:${c.color};">● ${c.itype}</span>${c.definition || ''}
    </div>`).join('');
}

async function loadAyah(suraid, verseid) {
  const r = await fetch(`/api/alifat/ayah?suraid=${suraid}&verseid=${verseid}`);
  const d = await r.json();
  if (d.error) return;
  document.getElementById('selSura').value = suraid;
  document.getElementById('selCase').value = verseid;
  renderAyah(d);
}

async function loadRandom() {
  const type = document.getElementById('selType').value || 'الكل';
  const r = await fetch(`/api/alifat/random?type=${encodeURIComponent(type)}`);
  const d = await r.json();
  if (!d.suraid) { showNotify('لا يوجد مواضع من هذا النوع.'); return; }
  await loadCasesForSura(d.suraid);
...
function openSelectionExplain(lo, hi) {
  ...
  const hit = currentRanges.find(r => lo <= r.hi && hi >= r.lo);
  currentWordCtx = { suraid: currentAyah.suraid, verseid: currentAyah.verseid, lo, hi, phrase };
  document.getElementById('wordTitle').textContent = phrase;
  document.getElementById('wordTitle').style.color = hit ? hit.case.color : '#F0C755';
  document.getElementById('wordContext').textContent =
    `سورة ${currentAyah.suraname} — الآية ${currentAyah.verseid}` + (hit ? ` — ${hit.case.itype}` : '');
  document.getElementById('wordResult').textContent = hit
    ? hit.case.definition
    : 'كلمة عادية — مواضع الألفات السبع في هذه الآية مظلَّلة بألوان مختلفة.';
...
function playClipReciter(reciter) {
  ...
  clipPlayer.src = '/api/alifat/clip?' + p.toString();
  clipPlayer.play().then(() => { status.textContent = ''; })
    .catch(async () => {
      try {
        const r = await fetch('/api/alifat/clip?' + p.toString());
...
async function askAI() {
  if (!currentWordCtx || !currentAyah) return;
  const { phrase, verseid } = currentWordCtx;
  ...
  try {
    const r = await fetch('/api/alifat/explain', {
      method: 'POST', headers: {'Content-Type':'application/json'},
      body: JSON.stringify({ word: phrase, sura: currentAyah.suraname, verse: verseid, question })
    });
    const j = await r.json();
...
(async function init() {
  ...
  await loadTypes();
  await loadSuras();
  const firstSura = document.getElementById('selSura').value;
  if (firstSura) {
    const cases = await loadCasesForSura(firstSura);
    if (cases.length) await loadAyah(firstSura, cases[0].verseid);
  }
})();
</script>
</body>
...
@bp.route('/api/alifat/suras')
def api_suras():
    conn = get_db(); cur = conn.cursor()
    try:
        cur.execute('SELECT DISTINCT suraid, suraname FROM alifatdirect ORDER BY suraid')
        rows = cur.fetchall()
    except Exception:
        rows = []
    conn.close()
...
@bp.route('/api/alifat/cases')
def api_cases():
    suraid = request.args.get('suraid', type=int)
    conn = get_db(); cur = conn.cursor()
    try:
        cur.execute('SELECT suraid,verseid,itype,klmat FROM alifatdirect WHERE suraid=? ORDER BY verseid,kseq', (suraid,))
        rows = cur.fetchall()
    except Exception:
        rows = []
    conn.close()
    return jsonify([{'suraid': r[0], 'verseid': r[1], 'itype': r[2], 'klmat': r[3]} for r in rows])

@bp.route('/api/alifat/types')
def api_types():
    conn = get_db(); cur = conn.cursor()
    try:
        cur.execute('SELECT itype, COUNT(*) FROM alifatdirect GROUP BY itype ORDER BY COUNT(*) DESC')
        rows = cur.fetchall()
    except Exception:
        rows = []
    conn.close()
    return jsonify([{'itype': r[0], 'count': r[1], 'color': get_color(r[0])} for r in rows])

@bp.route('/api/alifat/ayah')
def api_ayah():
    suraid  = request.args.get('suraid', type=int)
    verseid = request.args.get('verseid', type=int)
    d = _load_ayah(suraid, verseid)
    if not d:
        return jsonify({'error': 'لا توجد بيانات لهذا الموضع.'}), 404
    return jsonify(d)

@bp.route('/api/alifat/random')
def api_random():
    type_filter = request.args.get('type', 'الكل')
    suraid, verseid = _random_ayah(type_filter)
    return jsonify({'suraid': suraid, 'verseid': verseid})

@bp.route('/api/alifat/clip')
def api_clip():
...
@bp.route('/api/alifat/explain', methods=['POST'])
def api_explain():
    ...
    question = data.get('question', f'ما حكم الألف في {word}؟')

    API_KEY = os.environ.get('ANTHROPIC_API_KEY', '')
    try:
        system = ("You are an expert in Quran Tajweed and Uthmani orthography, specializing in "
                  "the seven 'alifat' (extra alifs written in the Uthmani script but pronounced "
                  "only when pausing: أنا، لكنا، الظنونا، الرسولا، السبيلا، قواريرا، سلاسلا). "
