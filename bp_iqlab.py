"""
iqlab_web_app.py — الإقلاب في القرآن الكريم (نسخة الويب، مستوى الصفحة)
نسخة Flask من iqlab_page_app.py، بنفس هوية ikhfa_web_app.py وizhar_web_app.py
البصرية. تنقل بالصفحة (نفس نمط iqlab_page_app.py الأصلي).
يعمل على بورت 5044.
"""
import os, re, sys, sqlite3, json
from flask import Flask, Blueprint, jsonify, request, send_from_directory, send_file

bp = Blueprint('iqlab', __name__)
...
# ══════════════════════════════════════════════════════════════
#  ألوان/أيقونات/تعريفات أنواع الإقلاب — منقولة حرفياً من iqlab_page_app.py
# ══════════════════════════════════════════════════════════════
C_GOLD2 = '#F0C755'
IQLAB_COLOR = {
    'نون ساكنة (كلمة واحدة)' : '#FF4081',
    'نون ساكنة (كلمتين)'     : '#00E5FF',
    'تنوين (كلمتين)'          : '#FFD700',
}
def get_color(itype):
    return IQLAB_COLOR.get(itype, C_GOLD2)

IQLAB_ICON = {
    'نون ساكنة (كلمة واحدة)' : '🔴',
    'نون ساكنة (كلمتين)'     : '🔵',
    'تنوين (كلمتين)'          : '🟡',
}
def get_icon(itype):
    return IQLAB_ICON.get(itype, '🔹')

IQLAB_DEFINITIONS = {
    'نون ساكنة (كلمة واحدة)': 'نون ساكنة تليها باء مباشرة في نفس الكلمة، مثل: أَنۢبِئۡهُم',
    'نون ساكنة (كلمتين)'    : 'نون ساكنة في آخر كلمة، تليها باء في أول الكلمة التالية، مثل: مِنۢ بَعۡدِ',
    'تنوين (كلمتين)'         : 'تنوين (فتح/ضم/كسر) في آخر كلمة، تليها باء في أول الكلمة التالية، مثل: سَمِيعٌۢ بَصِيرٌ',
}
...
#  iqlab_page_app.py دون أي تغيير في الاستعلامات
# ══════════════════════════════════════════════════════════════
def _load_page(page_num, type_filter=None):
    conn = get_db(); cur = conn.cursor()
    result = []
    try:
        params, where = [page_num], ['m.PAGENUM=?']
        if type_filter and type_filter != 'الكل':
            where.append('r.iqlab_type=?'); params.append(type_filter)
        ...
        for suraid, verseid, suraname, aya_text in ayas:
            ...
            if type_filter and type_filter != 'الكل':
                w2.append('iqlab_type=?'); p2.append(type_filter)
            cur.execute(f"""
                SELECT id, klma, klma2, position, iqlab_type, base_state, mark_position, letter
                FROM iqlab_rules WHERE {' AND '.join(w2)}
                ORDER BY position
            """, p2)
            rows = cur.fetchall()
            cases = []
            for cid, klma, klma2, position, itype, base_state, mark_pos, letter in rows:
                cases.append({
                    'id': cid, 'klma': (klma or '').strip(), 'klma2': (klma2 or '').strip(),
                    'position': position, 'itype': itype, 'base_state': base_state,
                    'mark_position': mark_pos, 'letter': letter,
                    'color': get_color(itype), 'icon': get_icon(itype),
                })
            result.append({
...
def _find_nearest_page(current_page, direction, type_filter=None):
    conn = get_db(); cur = conn.cursor()
    try:
        params, where = [], []
        if type_filter and type_filter != 'الكل':
        ...
        if direction == 1:
            cur.execute(f"""
                SELECT DISTINCT m.PAGENUM FROM iqlab_rules r
                JOIN mushafnew m ON m.SURAID=r.suraid AND m.VERSEID=r.verseid
...
def _random_page(type_filter=None):
    conn = get_db(); cur = conn.cursor()
    try:
        params, where = [], []
        if type_filter and type_filter != 'الكل':
...
<title>الإقلاب في القرآن الكريم</title>
<style>
:root {
...
<header>
  <div class="header-title">الإقلاب في القرآن الكريم</div>
  <div class="header-sub">نون ساكنة أو تنوين تُقلَب ميماً مخفاة عند حرف الباء</div>
  <div class="authors">
...
<div class="table-wrap">
  <div class="table-header" style="display:flex;justify-content:space-between;align-items:center;">
    <span>📋 كل حالات الإقلاب في هذه الصفحة</span>
    <button class="btn-showall" onclick="showAllCases()">⛶ عرض الكل</button>
  </div>
  <table>
    <thead><tr>
      <th>السورة</th><th>الآية</th><th>الكلمة الأولى</th><th>الكلمة الثانية</th><th>الحرف</th><th>النوع</th>
    </tr></thead>
...
<div id="allCasesOverlay" onclick="closeAllCases(event)">
  <div class="all-cases-modal">
    <div class="table-header" style="margin:-20px -20px 12px;border-radius:16px 16px 0 0;">📋 كل حالات الإقلاب في هذه الصفحة</div>
    <table>
      <thead><tr>
        <th>السورة</th><th>الآية</th><th>الكلمة الأولى</th><th>الكلمة الثانية</th><th>الحرف</th><th>النوع</th>
      </tr></thead>
...
const IQLAB_COLORS = {'نون ساكنة (كلمة واحدة)':'#FF4081','نون ساكنة (كلمتين)':'#00E5FF','تنوين (كلمتين)':'#FFD700'};
const IQLAB_DEFS = {
  'نون ساكنة (كلمة واحدة)': 'نون ساكنة تليها باء مباشرة في نفس الكلمة، مثل: أَنۢبِئۡهُم',
  'نون ساكنة (كلمتين)'    : 'نون ساكنة في آخر كلمة، تليها باء في أول الكلمة التالية، مثل: مِنۢ بَعۡدِ',
  'تنوين (كلمتين)'         : 'تنوين (فتح/ضم/كسر) في آخر كلمة، تليها باء في أول الكلمة التالية، مثل: سَمِيعٌۢ بَصِيرٌ',
};
const IQLAB_GENERAL_DEF = 'قلب النون الساكنة أو التنوين ميماً مخفاة عند ملاقاتها حرف الباء، مع بقاء الغنة وإخفاء الميم الناتجة بغنة كاملة مقدارها حركتان.';
...
function buildLegend() {
  document.getElementById('legend').innerHTML = Object.entries(IQLAB_COLORS).map(([name,color]) =>
  ...
  selType.innerHTML = '<option value="الكل">الكل</option>' +
    Object.keys(IQLAB_COLORS).map(n => `<option value="${n}">${n}</option>`).join('');
}
...
function renderPage(data) {
  ...
  if (!pageVerses.length) {
    container.innerHTML = '';
    showNotify('لا يوجد إقلاب موثّق في هذه الصفحة (أو حسب الفلتر الحالي) — استخدم ◄ أو ► للانتقال لأقرب صفحة.');
  ...
  pageVerses.forEach(v => {
    ...
    v.cases.forEach((c, ci) => {
      rows.push(`<tr class="case-row" data-key="${key}" data-caseidx="${ci}">
        ...
        <td style="font-weight:bold;color:var(--muted);">${c.letter || 'ب'}</td>
        <td style="color:${c.color};">${c.icon} ${c.itype}</td>
      </tr>`);
    });
  });
...
async function loadPage(page) {
  ...
  const r = await fetch(`/api/iqlab/page?page=${page}&type=${encodeURIComponent(type)}`);
  const d = await r.json();
  renderPage(d);
}
...
async function jumpPage(direction) {
  const type = document.getElementById('selType').value || 'الكل';
  const r = await fetch(`/api/iqlab/nearest?page=${currentPage}&dir=${direction}&type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadRandomPage() {
  const type = document.getElementById('selType').value || 'الكل';
  const r = await fetch(`/api/iqlab/random?type=${encodeURIComponent(type)}`);
  const d = await r.json();
  loadPage(d.page || 1);
}
...
function openSelectionExplain(key, lo, hi) {
  ...
  document.getElementById('wordResult').textContent =
    rulings.length ? (IQLAB_DEFS[rulings[0].itype] || IQLAB_GENERAL_DEF) : IQLAB_GENERAL_DEF;
...
function playClipReciter(reciter) {
  ...
  clipPlayer.src = '/api/iqlab/clip?' + p.toString();
  clipPlayer.play().then(() => { status.textContent = ''; })
    .catch(async () => {
      try {
        const r = await fetch('/api/iqlab/clip?' + p.toString());
...
async function askAI() {
  ...
  try {
    const r = await fetch('/api/iqlab/explain', {
...
@bp.route('/api/iqlab/page')
def api_page():
    page = request.args.get('page', 1, type=int)
    type_filter = request.args.get('type', 'الكل')
    verses = _load_page(page, type_filter)
    return jsonify({'page': page, 'verses': verses})

@bp.route('/api/iqlab/nearest')
def api_nearest():
    page      = request.args.get('page', 1, type=int)
    direction = request.args.get('dir', 1, type=int)
    type_filter = request.args.get('type', 'الكل')
    return jsonify({'page': _find_nearest_page(page, direction, type_filter)})

@bp.route('/api/iqlab/random')
def api_random():
    type_filter = request.args.get('type', 'الكل')
    return jsonify({'page': _random_page(type_filter)})

@bp.route('/api/iqlab/clip')
def api_clip():
...
@bp.route('/api/iqlab/explain', methods=['POST'])
def api_explain():
    ...
    question = data.get('question', f'ما حكم إقلاب كلمة {word} في القرآن الكريم؟')
    ...
    try:
        system = ("You are an expert in Quran Tajweed specializing in Iqlab (conversion to meem) rules. "
