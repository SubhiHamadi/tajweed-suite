...
def get_color(madd_type):
    mt = (madd_type or '').strip()
    for k, v in ITYPE_COLORS.items():
        if k in mt or mt in k: return v
    return '#F48FB1'
...
<title>أحكام مد الصلة الصغرى في القرآن الكريم</title>
<style>
:root {
  --navy:  #00BCD4;
  --navy2: #0A2040;
...
.header-sub { font-size:clamp(10px,2.8vw,13px); color:#F48FB1; margin-bottom:10px; }
...
.author-info { color:#F48FB1; font-size:0.88em; }
...
.leg-sila { background:rgba(0,229,255,0.1); color:var(--sila); border:2px solid var(--sila); }
...
.help-tab-soghra-def.active { background:var(--sila); color:#020B18; border-color:var(--sila); }
.help-tab-soghra-ex.active { background:var(--sila); color:#020B18; border-color:var(--sila); }
...
<header>
  <div class="header-title">أحكام مد الصلة الصغرى في القرآن الكريم</div>
  <div class="header-sub">مد الصلة الصغرى — هاء الضمير المتحركة بين حرفين متحركين والثاني ليس همزة</div>
  <div class="authors">
...
<div class="controls">
  ...
  <div class="ctrl-row" style="justify-content:center;">
    <button class="btn-toggle" onclick="showHelp()" style="background:#EDE7F6;color:#512DA8;border:1.5px solid #9575CD;padding:8px 24px;">📚 تعريفات مد الصلة الصغرى</button>
...
<div class="legend">
  <div class="legend-item" id="leg-مد الصلة الصغرى" style="background:rgba(244,143,177,0.1);color:#F48FB1;border:2px solid #F48FB1" onclick="filterByType('مد الصلة الصغرى',this)">● مد الصلة الصغرى</div>
</div>

<!-- نافذة التعريفات -->
<div class="help-overlay" id="helpOverlay" onclick="hideHelp(event)">
  <div class="help-modal">
    <div class="help-title">📚 تعريفات مد الصلة الصغرى</div>
    <div style="font-size:13px;color:#0D47A1;font-weight:700;text-align:center;margin-bottom:12px;padding:8px;background:rgba(255,255,255,0.92);border-radius:8px;">
      هاء الضمير المتحرّكة للغائب المفرد المذكّر بين حرفين متحرّكين، والحرف الذي يلي الهاء ليس همزة — يُمدّ 2 حركة
    </div>
    <div class="help-tabs">
            <button class="help-tab" style="border-color:#F48FB1;color:#F48FB1" onclick="showTab('soghra-def')">تعريف</button>
      <button class="help-tab" style="border-color:#F48FB1;color:#F48FB1" onclick="showTab('soghra-ex')">أمثلة</button>
    </div>

            <div class="hcontent show" id="tab-soghra-def">
      <div class="help-def"><b style="color:#F48FB1">مد الصلة الصغرى</b><br>
        هاء الضمير المتحركة (للمذكر الغائب المفرد) بين حرفين متحرّكين،
        بشرط أن يكون الحرف الذي يلي الهاء غير همزة.</div>
      <div class="help-letters" style="color:#F48FB1">مقدار المد: 2 حركة (مد طبيعي)</div>
      <div class="help-example"><div class="help-ex-label">الشرط:</div>
        ✅ الحرف قبل الهاء متحرّك<br>
        ✅ الهاء نفسها متحرّكة (مضمومة أو مكسورة)<br>
        ✅ يليها همزة في الكلمة التالية</div>
    </div>
    <div class="hcontent" id="tab-soghra-ex">
      <div class="help-def"><b style="color:#F48FB1">أمثلة من القرآن الكريم</b></div>
      <div class="help-example"><div class="help-ex-label">صلة بالواو (هاء مضمومة):</div>
        ﴿حَوۡلَهُۥ﴾ · ﴿إِنَّهُۥ﴾ · ﴿وَرَحۡمَتُهُۥ﴾</div>
      <div class="help-example"><div class="help-ex-label">صلة بالياء (هاء مكسورة):</div>
        ﴿بِهِۦ﴾ · ﴿لِقَوۡمِهِۦ﴾ · ﴿مِّثۡلِهِۦ﴾</div>
    </div>
      <div class="help-letters" style="color:#F48FB1">مقدار المد: 2 حركة</div>
      <div class="help-example"><div class="help-ex-label">أمثلة:</div>
        ﴿فِي أَنفُسِكُمۡ﴾ · ﴿الَّذِي أَنزَلَ﴾</div>
    </div>
      <div class="help-letters" style="color:#69F0AE">مقدار المد: 2 حركة</div>
...
<div class="table-wrap">
  <div class="table-header">قائمة حالات مد الصلة الصغرى المستخرجة</div>
  <table>
    <thead>
      <tr>
        <th>ت</th>
...
<div id="allCasesOverlay" onclick="closeAllCases(event)">
  <div id="allCasesModal">
    <div id="allCasesTitle">📋 كل حالات مد الصلة الصغرى في هذه الصفحة</div>
...
function showAllCases() {
  if (!currentData || !currentData.cases || !currentData.cases.length) {
    alert('لا يوجد مد الصلة الصغرى موثّق في هذه الآية.');
    return;
  }
...
async function changeAyah(delta) {
  ...
  try {
    const r = await fetch(`/api/soghra/nextayah?suraid=${currentData.suraid}&verseid=${currentData.verseid}&delta=${delta}&type=${encodeURIComponent(type)}`);
    const d = await r.json();
    showAyahNavLoading(false);
    if (d.found) {
      const p = {suraid: d.suraid, verseid: d.verseid};
      if (type !== 'الكل') p.type = type;
      const r2 = await fetch('/api/soghra/ayah?' + new URLSearchParams(p));
...
async function loadSuras(type) {
  ...
  const r = await fetch(`/api/soghra/suras?type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadVerses() {
  ...
  if (!suraid) return;
  const type   = activeFilter || 'الكل';
  const r = await fetch(`/api/soghra/verses?suraid=${suraid}&type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadRandom() {
  ...
  const p = {random:1};
  if (type !== 'الكل') p.type = type;
  const r = await fetch('/api/soghra/ayah?' + new URLSearchParams(p));
  const d = await r.json();
  if (d.error) return;
  currentData = d;
  renderAyah(d);
...
function renderAyah(d) {
  ...
  // العدادات
  const counts = {'مد الصلة الصغرى':0};
  d.cases.forEach(c => { if (counts[c.itype] !== undefined) counts[c.itype]++; });
  const styleMap = {
    'مد الصلة الصغرى': 'background:#FCE4EC;color:#F48FB1;border:1px solid #F48FB1',
  };
  ...
  // تلوين الكلمات — مد الصلة الصغرى: kseq يشير لكلمة هاء الضمير (كلمة واحدة فقط)
...
function playClipReciter(reciter) {
  ...
  clipPlayer.src = '/api/soghra/clip?' + p.toString();
  clipPlayer.play().then(() => { status.textContent = ''; })
    .catch(async () => {
      try {
        const r = await fetch('/api/soghra/clip?' + p.toString());
        const d = await r.json();
...
const WORD_QUESTIONS = {
  1: 'ما حكم هذه الكلمة من أحكام مد الصلة الصغرى؟',
...
async function askWordQuestion(num) {
  ...
  try {
    const r = await fetch('/api/soghra/explain', {
...
def api_nextayah():
    ...
    try:
        ...
        if itype and itype != 'الكل':
            cur.execute('''SELECT DISTINCT suraid, verseid FROM md_soghra
                           WHERE rem1=?
...
@bp.route('/api/soghra/suras')
def api_suras():
    itype = request.args.get('type','الكل')
    conn = get_db(); cur = conn.cursor()
    try:
        if itype and itype != 'الكل':
            cur.execute('''SELECT DISTINCT suraid, suraname FROM md_soghra
                           WHERE rem1=? ORDER BY CAST(suraid AS INTEGER)''', (itype,))
        else:
            cur.execute('SELECT DISTINCT suraid, suraname FROM md_soghra ORDER BY CAST(suraid AS INTEGER)')
        rows = [{'suraid':r[0],'suraname':r[1]} for r in cur.fetchall()]
    except: rows = []
    conn.close(); return jsonify(rows)


@bp.route('/api/soghra/verses')
def api_verses():
    ...
    try:
        if itype and itype != 'الكل':
            cur.execute('''SELECT DISTINCT verseid FROM md_soghra WHERE suraid=? AND rem1=?
                           ORDER BY CAST(verseid AS INTEGER)''', (suraid, itype))
        else:
            cur.execute('''SELECT DISTINCT verseid FROM md_soghra WHERE suraid=? ORDER BY CAST(verseid AS INTEGER)''', (suraid,))
        rows = cur.fetchall()
    except: rows = []
    conn.close(); return jsonify([r[0] for r in rows])


@bp.route('/api/soghra/ayah')
def api_ayah():
    suraid  = request.args.get('suraid')
    ...
    if rand == '1':
        try:
            if itype and itype != 'الكل':
                cur.execute('SELECT DISTINCT suraid,verseid FROM md_soghra WHERE rem1=? ORDER BY RANDOM() LIMIT 1', (itype,))
            else:
                cur.execute('SELECT DISTINCT suraid,verseid FROM md_soghra ORDER BY RANDOM() LIMIT 1')
            r = cur.fetchone()
    ...
    # جلب نص الآية من mushafnew (ayahtext في md_soghra يحتوي اسم السورة فقط)
    try:
    ...
    cases = [{
        'word'    : c[0],
        'itype'   : 'مد الصلة الصغرى',   # تطبيع rem1 للتوحيد
        'length'  : c[2],
        'position': c[3],
        'color'   : get_color(c[1])
    } for c in cases_raw]
...
@bp.route('/api/soghra/explain', methods=['POST'])
def api_explain():
    ...
    try:
        system = ("You are an expert in Quran Tajweed specializing in Madd Al-Muttasil (Connected Madd) rules. "
                  ...
                  "If the database says it is Madd Al-Sila Al-Sughra, then it IS Madd Al-Sila Al-Sughra - do not deny this. "
                  ...
                  "FACT: Madd Al-Sila Al-Sughra is 2 harakaat (natural madd).")
...
@bp.route('/api/soghra/clip')
def api_clip():
    """يُرجع مقطعاً صوتياً دقيقاً (مجال كلمات متتالية) من تلاوة أحد
    الشيوخ الثلاثة، بالاعتماد على word_audio_helper."""
