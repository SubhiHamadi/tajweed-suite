...
<title>أحكام مد الصلة الكبرى في القرآن الكريم</title>
<style>
:root {
  --navy:  #00BCD4;
  --navy2: #0A2040;
...
.help-tab-sila-def.active { background:var(--sila); color:#020B18; border-color:var(--sila); }
.help-tab-sila-ex.active { background:var(--sila); color:#020B18; border-color:var(--sila); }
...
<header>
  <div class="header-title">أحكام مد الصلة الكبرى في القرآن الكريم</div>
  <div class="header-sub">مد الصلة الكبرى — هاء الضمير المتحركة بين حرفين متحركين والثاني همزة</div>
  <div class="authors">
...
<div class="controls">
  ...
  <div class="ctrl-row" style="justify-content:center;">
    <button class="btn-toggle" onclick="showHelp()" style="background:#EDE7F6;color:#512DA8;border:1.5px solid #9575CD;padding:8px 24px;">📚 تعريفات مد الصلة الكبرى</button>
...
<div class="legend">
  <div class="legend-item" id="leg-مد الصلة الكبرى" style="background:rgba(0,229,255,0.1);color:#00E5FF;border:2px solid #00E5FF" onclick="filterByType('مد الصلة الكبرى',this)">● مد الصلة الكبرى</div>
</div>

<!-- نافذة التعريفات -->
<div class="help-overlay" id="helpOverlay" onclick="hideHelp(event)">
  <div class="help-modal">
    <div class="help-title">📚 تعريفات مد الصلة الكبرى</div>
    <div style="font-size:13px;color:#0D47A1;font-weight:700;text-align:center;margin-bottom:12px;padding:8px;background:rgba(255,255,255,0.92);border-radius:8px;">
      هاء الضمير المتحرّكة للغائب المفرد المذكّر بين حرفين متحرّكين، والحرف الذي يلي الهاء همزة — يُمدّ 4 أو 5 حركات
    </div>
    <div class="help-tabs">
            <button class="help-tab" style="border-color:#00E5FF;color:#00E5FF" onclick="showTab('sila-def')">تعريف</button>
      <button class="help-tab" style="border-color:#00E5FF;color:#00E5FF" onclick="showTab('sila-ex')">أمثلة</button>
    </div>

            <div class="hcontent show" id="tab-sila-def">
      <div class="help-def"><b style="color:#00E5FF">مد الصلة الكبرى</b><br>
        هاء الضمير المتحركة (للمذكر الغائب المفرد) بين حرفين متحرّكين،
        بشرط أن يكون الحرف الذي يلي الهاء همزةً في الكلمة التالية.</div>
      <div class="help-letters" style="color:#00E5FF">مقدار المد: 4 أو 5 حركات (2 عند بعض القراء)</div>
    ...
    <div class="hcontent" id="tab-sila-ex">
      <div class="help-def"><b style="color:#00E5FF">أمثلة من القرآن الكريم</b></div>
      <div class="help-example"><div class="help-ex-label">صلة بالواو (هاء مضمومة):</div>
        ﴿رَبُّهُۥٓ أَسۡلِمۡ﴾ · ﴿فَلَهُۥٓ أَجۡرُهُۥ﴾ · ﴿عِندَهُۥٓ إِلَّا﴾</div>
      <div class="help-example"><div class="help-ex-label">صلة بالياء (هاء مكسورة):</div>
        ﴿بِهِۦٓ إِلَّا﴾ · ﴿لِقَوۡمِهِۦٓ إِنَّ﴾ · ﴿عِلۡمِهِۦٓ إِلَّا﴾</div>
    </div>
      <div class="help-letters" style="color:#00E5FF">مقدار المد: 4 أو 5 حركات</div>
...
<div class="table-wrap">
  <div class="table-header">قائمة حالات مد الصلة الكبرى المستخرجة</div>
  <table>
    <thead>
      <tr>
        <th>ت</th>
...
<div id="allCasesOverlay" onclick="closeAllCases(event)">
  <div id="allCasesModal">
    <div id="allCasesTitle">📋 كل حالات مد الصلة الكبرى في هذه الصفحة</div>
...
function showAllCases() {
  if (!currentData || !currentData.cases || !currentData.cases.length) {
    alert('لا يوجد مد الصلة الكبرى موثّق في هذه الآية.');
    return;
  }
...
async function changeAyah(delta) {
  ...
  try {
    const r = await fetch(`/api/sila/nextayah?suraid=${currentData.suraid}&verseid=${currentData.verseid}&delta=${delta}&type=${encodeURIComponent(type)}`);
    const d = await r.json();
    showAyahNavLoading(false);
    if (d.found) {
      const p = {suraid: d.suraid, verseid: d.verseid};
      if (type !== 'الكل') p.type = type;
      const r2 = await fetch('/api/sila/ayah?' + new URLSearchParams(p));
...
async function loadSuras(type) {
  ...
  const r = await fetch(`/api/sila/suras?type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadVerses() {
  ...
  if (!suraid) return;
  const type   = activeFilter || 'الكل';
  const r = await fetch(`/api/sila/verses?suraid=${suraid}&type=${encodeURIComponent(type)}`);
  const d = await r.json();
...
async function loadRandom() {
  ...
  const p = {random:1};
  if (type !== 'الكل') p.type = type;
  const r = await fetch('/api/sila/ayah?' + new URLSearchParams(p));
  const d = await r.json();
  if (d.error) return;
  currentData = d;
  renderAyah(d);
...
function renderAyah(d) {
  ...
  // العدادات
  const counts = {'مد الصلة الكبرى':0};
  d.cases.forEach(c => { if (counts[c.itype] !== undefined) counts[c.itype]++; });
  const styleMap = {
    'مد الصلة الكبرى': 'background:#E0F7FA;color:#00E5FF;border:1px solid #00E5FF',
  };
  ...
  // تلوين الكلمات — مد الصلة الكبرى: kseq يشير لكلمة هاء الضمير، وkseq+1 للكلمة التي تبدأ بهمزة
...
function playClipReciter(reciter) {
  ...
  clipPlayer.src = '/api/sila/clip?' + p.toString();
  clipPlayer.play().then(() => { status.textContent = ''; })
    .catch(async () => {
      try {
        const r = await fetch('/api/sila/clip?' + p.toString());
        const d = await r.json();
...
const WORD_QUESTIONS = {
  1: 'ما حكم هذه الكلمة من أحكام مد الصلة الكبرى؟',
...
async function askWordQuestion(num) {
  ...
  try {
    const r = await fetch('/api/sila/explain', {
...
def api_nextayah():
    ...
    try:
        ...
        if itype and itype != 'الكل':
            cur.execute('''SELECT DISTINCT suraid, verseid FROM md_sila_kubra
                           WHERE rem1=?
...
@bp.route('/api/sila/suras')
def api_suras():
    itype = request.args.get('type','الكل')
    conn = get_db(); cur = conn.cursor()
    try:
        if itype and itype != 'الكل':
            cur.execute('''SELECT DISTINCT suraid, suraname FROM md_sila_kubra
                           WHERE rem1=? ORDER BY CAST(suraid AS INTEGER)''', (itype,))
        else:
            cur.execute('SELECT DISTINCT suraid, suraname FROM md_sila_kubra ORDER BY CAST(suraid AS INTEGER)')
        rows = [{'suraid':r[0],'suraname':r[1]} for r in cur.fetchall()]
    except: rows = []
    conn.close(); return jsonify(rows)


@bp.route('/api/sila/verses')
def api_verses():
    ...
    try:
        if itype and itype != 'الكل':
            cur.execute('''SELECT DISTINCT verseid FROM md_sila_kubra WHERE suraid=? AND rem1=?
                           ORDER BY CAST(verseid AS INTEGER)''', (suraid, itype))
        else:
            cur.execute('''SELECT DISTINCT verseid FROM md_sila_kubra WHERE suraid=? ORDER BY CAST(verseid AS INTEGER)''', (suraid,))
        rows = cur.fetchall()
    except: rows = []
    conn.close(); return jsonify([r[0] for r in rows])


@bp.route('/api/sila/ayah')
def api_ayah():
    suraid  = request.args.get('suraid')
    ...
    if rand == '1':
        try:
            if itype and itype != 'الكل':
                cur.execute('SELECT DISTINCT suraid,verseid FROM md_sila_kubra WHERE rem1=? ORDER BY RANDOM() LIMIT 1', (itype,))
            else:
                cur.execute('SELECT DISTINCT suraid,verseid FROM md_sila_kubra ORDER BY RANDOM() LIMIT 1')
            r = cur.fetchone()
    ...
    try:
        cur.execute('''SELECT suraname, ayahtext, pagenum
                       FROM md_sila_kubra WHERE suraid=? AND verseid=? LIMIT 1''',
                    (suraid, verseid))
...
@bp.route('/api/sila/explain', methods=['POST'])
def api_explain():
    ...
    try:
        system = ("You are an expert in Quran Tajweed specializing in Madd Al-Muttasil (Connected Madd) rules. "
                  ...
                  "If the database says it is Madd Al-Sila Al-Kubra, then it IS Madd Al-Sila Al-Kubra - do not deny this. "
                  ...
                  "FACT: Madd Al-Sila Al-Kubra is 4 or 5 harakaat (2 harakaat for some reciters).")
